...
# Inicializa o histórico na memória da sessão (Session State)
...
# CSS Bloomberg - Estilo Total Black
st.markdown("""
    <style>
    ...
    .history-table th { color: #888; text-transform: uppercase; }
    </style>
...
def get_live_data(ticker):
    try:
        # Download rápido (apenas o dia de hoje)
...
with placeholder.container():
    ...
    if not spot_df.empty:
        spot_at = spot_df['Close'].iloc[-1].item() # Preço atual
        time_now = datetime.now().strftime("%H:%M:%S")
        
        # Lógica de Histórico: Só adiciona se o preço mudou
        if not st.session_state.history or st.session_state.history[0]['Preço'] != spot_at:
            ...
            # Mantém apenas os últimos 5 registos
        ...
        c2.metric("DXY", f"{dxy_df['Close'].iloc[-1].item():.2f}", f"{v_dxy:.2f}%")
        c3.metric("EWZ", f"{ewz_df['Close'].iloc[-1].item():.2f}", f"{v_ewz:.2f}%")
        ...
        # TABELA DE HISTÓRICO (TAPE READING)
        ...
        for i in range(len(st.session_state.history)):
            ...
            if i < len(st.session_state.history) - 1:
                if row['Preço'] > st.session_state.history[i+1]['Preço']:
                    sentido = "<span style='color:#00FF00;'>▲</span>"
                elif row['Preço'] < st.session_state.history[i+1]['Preço']:
                    sentido = "<span style='color:#FF0000;'>▼</span>"
