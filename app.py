...
# 4. CSS CUSTOMIZADO (TITULO, SPOT E VELOCÍMETRO)
st.markdown("""
<style>
    ...
    .t-header { text-align: center; padding: 20px 0 5px 0; }
    .t-light { font-weight: 300; color: #fff; font-size: 24px; letter-spacing: 4px; }
    .t-bold { font-weight: 900; color: #fff; font-size: 24px; letter-spacing: 4px; }
    
    /* SPOT E VARIAÇÃO */
    .spot-container { text-align: center; margin-bottom: 10px; }
    .spot-val { font-family: 'Chakra Petch'; font-size: 32px; font-weight: 700; color: #fff; }
    .spot-var { font-family: 'Chakra Petch'; font-size: 16px; margin-left: 10px; }

    /* VELOCÍMETRO (TERMÔMETRO) */
    .v-frame { width: 60%; height: 6px; background: #1a1a1a; border-radius: 10px; margin: 10px auto; overflow: hidden; border: 1px solid #333; }
    .v-bar { height: 100%; transition: width 0.8s ease-in-out; box-shadow: 0 0 12px currentColor; }
    ...
    .d-row { display: flex; justify-content: space-between; align-items: center; padding: 16px 15px; border-bottom: 1px solid #111; }
...
if st.session_state.user_type == "ADM":
    with st.expander("PAINEL DE CONTROLE"):
        with st.form("adm"):
...
while True:
    ...
    if d_m["last"] > 0:
        ...
        # VELOCÍMETRO (Distância do Equilíbrio)
        dist_pts = abs(spot - equi) * 1000
        v_width = min(dist_pts * 4, 100) 
        ...
        with ui_area.container():
            ...
            # RODAPÉ
            btk = f"<span class='tk-item'><b>SPOT</b> {spot:.4f} ({spot_var:+.2f}%)</span> <span class='tk-item'><b>DXY</b> {d_m['last']:.2f}</span> <span class='tk-item'><b>EWZ</b> {e_m['last']:.2f}</span> <span class='tk-item'><b>EURUSD</b> {eu_m['last']:.4f}</span> <span class='tk-item'><b>SPREAD</b> {spr:+.2f}%</span> <span class='tk-item'><b>MURAL:</b> {st.session_state.mural}</span>"
