...
# 2. ESTADOS DO SISTEMA (VARIÁVEIS)
...
if 'txt_topo' not in st.session_state: st.session_state.txt_topo = "RESPEITE O SEU GERENCIAMENTO"
if 'show_settings' not in st.session_state: st.session_state.show_settings = False

# 3. ESTILO CSS COMPLETO
st.markdown("""
<style>
    ...
    .tk-move { white-space: nowrap; animation: move 25s linear infinite; display: flex; align-items: center; }
...
# 4. BOTÃO SET / INTERFACE DE AJUSTE
col_btn, _ = st.columns([1, 8])
if col_btn.button("⚙️ SET" if not st.session_state.show_settings else "✖ FECHAR"):
    st.session_state.show_settings = not st.session_state.show_settings
    st.rerun()

if st.session_state.show_settings:
    with st.container():
        st.write("### AJUSTES DO TERMINAL")
        ...
        st.session_state.v22 = v1.number_input("VAR MIN (Ex: 0.0220)", value=st.session_state.v22, format="%.4f")
        st.session_state.v31 = v2.number_input("VAR JUSTO (Ex: 0.0310)", value=st.session_state.v31, format="%.4f")
        st.session_state.v42 = v3.number_input("VAR MAX (Ex: 0.0420)", value=st.session_state.v42, format="%.4f")
        
        st.session_state.txt_topo = st.text_input("FRASE ACIMA DO TICKER", value=st.session_state.txt_topo)
        
        if st.button("SALVAR ALTERAÇÕES"):
            st.session_state.show_settings = False
            st.rerun()

# 5. CÁLCULOS E DADOS EM TEMPO REAL
...
while True:
    try:
        ...
        p_var = ((p_atual / info.previous_close) - 1) * 100
        
        dx = yf.Ticker("DX-Y.NYB").fast_info
        dx_p, dx_v = dx.last_price, ((dx.last_price / dx.previous_close) - 1) * 100
    except:
        p_atual, p_var, dx_p, dx_v = 0, 0, 0, 0

    if p_atual > 0:
        memoria = (st.session_state.ptax + st.session_state.fech) / 2
        diff_mem = ((p_atual / memoria) - 1) * 100
        gauge_angle = max(min(diff_mem * 140, 90), -90)
        
        # Lógica do Botão Alerta
        if diff_mem > 0.10: 
            alerta_t, alerta_c = "PRECIFICAÇÃO DE ALTA", "#00ff88"
        elif diff_mem < -0.10: 
            alerta_t, alerta_c = "PRECIFICAÇÃO DE BAIXA", "#ff3333"
        else: 
            alerta_t, alerta_c = "PRECIFICAÇÃO NEUTRA", "#ffff00"
        ...
        with main_placeholder.container():
            ...
            st.markdown(f"""
                <div class='t-header'>
                    ...
                    <div style='font-family:Chakra Petch; font-size:22px; font-weight:700;'>
                        {p_atual:.4f} <span style='color:{"#00ff88" if p_var >= 0 else "#ff3333"}'>{p_var:+.2f}%</span>
                    </div>
                    <div class="gauge-container">
                        <div class="gauge-bg"></div><div class="gauge-cover"></div>
                        <div class="gauge-needle" style="transform: translateX(-50%) rotate({gauge_angle}deg);"></div>
                    </div>
                    <div class="btn-alerta" style="border: 1px solid {alerta_c}; color: {alerta_c};">{alerta_t}</div>
                </div>
                ...
                <div class="d-row">
                    ...
                    <div style="display:flex; gap:15px;">
                        <div style="text-align:center"><small>MIN</small><br><span style="color:#ff3333">{(round((p_atual+st.session_state.v22)*2000)/2000):.4f}</span></div>
                        <div style="text-align:center"><small>JUSTO</small><br><span style="color:#0066cc">{(round((p_atual+st.session_state.v31)*2000)/2000):.4f}</span></div>
                        <div style="text-align:center"><small>MAX</small><br><span style="color:#00ff88">{(round((p_atual+st.session_state.v42)*2000)/2000):.4f}</span></div>
                    </div>
                </div>
                ...
                <div class="d-row">
                    ...
                    <div style="display:flex; gap:40px;">
                        <div class="corr-box">
                            ...
                            <span class="val-22">{(st.session_state.ref-st.session_state.v22):.4f}</span>
                        </div>
                        <div class="corr-box">
                            <span class="val-11">{(st.session_state.ref+0.0110):.4f}</span>
                            <span class="val-22">{(st.session_state.ref+st.session_state.v22):.4f}</span>
            ...
            ticker_html = f"""
                <span class='tk-item'>SPOT: {p_atual:.4f} <span style='color:{c_tk(p_var)}'>({p_var:+.2f}%)</span></span>
                <span class='tk-item'>DXY: {dx_p:.2f} <span style='color:{c_tk(dx_v)}'>({dx_v:+.2f}%)</span></span>
                <span class='tk-item'>PTAX: {st.session_state.ptax:.4f}</span>
            """
