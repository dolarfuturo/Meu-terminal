...
<style>
    ...
    .t-title { font-size: 22px; letter-spacing: 5px; font-weight: 300; color: #fff; } /* TERMINAL FINO */
    .t-bold { font-weight: 900; } /* DÓLAR NEGRITO */
    .t-line { width: 60%; height: 1px; background: #333; margin: 5px auto 10px auto; }
    
    /* VELOCÍMETRO CSS PURO */
    ...
    .gauge-needle { position: absolute; bottom: 0; left: 50%; width: 2px; height: 70px; background: #fff; transform-origin: bottom center; transition: transform 0.8s ease; z-index: 10; }
...
while True:
    ...
    if s_p > 0:
        ...
        spr = dx_v - ew_v
        justo_spot = round((s_p + 0.0310) * 2000) / 2000
        ...
        with main_ui.container():
            ...
            st.markdown(f"""
                <div class='t-header'>
                    ...
                    <div style='font-family:Chakra Petch; font-size:18px; font-weight:700; margin-bottom: 5px;'>
            ...
            st.markdown(f"""<div class="d-row"><div class="d-label">PREÇO JUSTO SPOT</div><div style="display:flex; gap:15px;">
                ...
                <div style="text-align:center"><small>JUSTO</small><br><span style="font-family:Chakra Petch; font-weight:700; font-size:16px; color:#0066cc">{justo_spot:.4f}</span></div>
