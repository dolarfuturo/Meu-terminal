...
# 2. INICIALIZAÇÃO DE VARIÁVEIS (VALORES DA SUA MATRIZ)
...
# 3. CSS (NEGRITOS E CORES CONFORME SOLICITADO)
st.markdown("""
<style>
...
if st.session_state.show_settings:
    ...
    with c_p:
        ...
        st.session_state.ref = st.number_input("REF (BASE REF. INST)", value=st.session_state.ref, format="%.4f")
    with c_v:
...
while True:
    if not st.session_state.show_settings:
        try:
            ...
            spot = s_m.last_price
            spr_pct = ((d_m.last_price / d_m.previous_close) - 1) * 100 - ((e_m.last_price / e_m.previous_close) - 1) * 100
        except:
            spot = spr_pct = 0

        if spot > 0:
            # --- CÁLCULOS MATRIZ ---
            # 1. Paridade Global (Ajuste corrigido pelo Spread)
            paridade = st.session_state.ajuste * (1 + (spr_pct / 100))
            
            # 2. Equilíbrio (REF + 22 pts arredondado)
            equi = round((st.session_state.ref + st.session_state.v22) * 2000) / 2000
            
            # 3. Preço Justo (Spot + variações)
            justo_min = round((spot + st.session_state.v22) * 2000) / 2000
            justo_med = round((spot + st.session_state.v31) * 2000) / 2000
            justo_max = round((spot + st.session_state.v42) * 2000) / 2000
            
            # 4. Ref Institucional (Baseado na variável REF do SET)
            ref_min = round((st.session_state.ref + st.session_state.v22) * 2000) / 2000
            ref_med = round((st.session_state.ref + st.session_state.v31) * 2000) / 2000
            ref_max = round((st.session_state.ref + st.session_state.v42) * 2000) / 2000
            
            # Gauge e Alerta (Baseado na distância do Justo)
            diff_p = spot - justo_med
            ...
            with placeholder.container():
                st.markdown(f"""
                    ...
                    <div class="d-row"><div class="d-label">PARIDADE GLOBAL</div><div class="d-value" style="color:#cc9900">{paridade:.4f}</div></div>
                    
                    <div class="d-row"><div class="d-label">PREÇO JUSTO</div><div style="display:flex; gap:15px;">
                        <div style="text-align:center"><small>MIN</small><br><span style="color:#cc3333" class="d-value">{justo_min:.4f}</span></div>
                        <div style="text-align:center"><small>JUSTO</small><br><span style="color:#0066cc" class="d-value">{justo_med:.4f}</span></div>
                        <div style="text-align:center"><small>MAX</small><br><span style="color:#00cc66" class="d-value">{justo_max:.4f}</span></div>
                    </div></div>

                    <div class="d-row"><div class="d-label">REF. INSTITUCIONAL</div><div style="display:flex; gap:15px;">
                        <div style="text-align:center"><small>MIN</small><br><span style="color:#cc3333" class="d-value">{ref_min:.4f}</span></div>
                        <div style="text-align:center"><small>JUSTO</small><br><span style="color:#0066cc" class="d-value">{ref_med:.4f}</span></div>
                        <div style="text-align:center"><small>MAX</small><br><span style="color:#00cc66" class="d-value">{ref_max:.4f}</span></div>
                    </div></div>

                    <div class="d-row" style="border-bottom:none"><div class="d-label">REGIÕES DE CORREÇÃO</div>
                ...
                # RODAPÉ (TICKER)
                tk_data = [
                    ("DXY", d_m.last_price, ((d_m.last_price/d_m.previous_close)-1)*100),
                    ("EWZ", e_m.last_price, ((e_m.last_price/e_m.previous_close)-1)*100),
                    ("EURUSD", eu_m.last_price, ((eu_m.last_price/eu_m.previous_close)-1)*100)
                ]
                def c_tk(v): return "#00ff88" if v >= 0 else "#ff3333"
                tk_str = "".join([f"<span class='tk-item'><b>{n}</b> {p:.2f} <span style='color:{c_tk(v)}'>({v:+.2f}%)</span></span>" for n,p,v in tk_data])
                tk_str += f"<span class='tk-item'><b>SPREAD</b> <span style='color:#ffff00'>{spr_pct:+.2f}%</span></span>"
                tk_str += f"<span class='tk-item'><b>PTAX</b> {st.session_state.ptax:.4f}</span>"
                
                st.markdown(f"<div class='f-bar'><div class='tk-move'>{tk_str} {tk_str}</div></div>", unsafe_allow_html=True)
