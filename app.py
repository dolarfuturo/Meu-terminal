...
while True:
    ...
    if d_m["last"] > 0:
        ...
        with ui_area.container():
            ...
            <span class='tk-item'><b>DXY</b> {d_m['last']:.2f} <span style="color:{f_color(d_m['var'])}">({d_m['var']:+.2f}%)</span></span>
            <span class='tk-item'><b>EWZ</b> {e_m['last']:.2f} <span style="color:{f_color(e_m['var'])}">({e_m['var']:+.2f}%)</span></span>
            <span class='tk-item'><b>EURUSD</b> {eu_m['last']:.4f} <span style="color:{f_color(eu_m['var'])}">({eu_m['var']:+.2f}%)</span></span>
            <span class='tk-item'><b>SPREAD</b> <span style="color:{f_color(spr)}">{spr:+.2f}%</span></span>
            """
