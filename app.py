...
st.markdown("""
    ...
    * { font-family: 'Roboto Mono', monospace !important; text-transform: uppercase; }
    ...
    .stPopover button { background-color: #111 !important; color: #555 !important; border: 1px solid #222 !important; font-size: 10px !important; height: 25px !important; }
    </style>
...
def fetch_data(ticker, premarket=False):
    try:
        # Força busca de pré-mercado para tickers americanos (EWZ, DXY)
        data = yf.download(ticker, period="1d", interval="1m", progress=False, prepost=premarket)
        return data if (data is not None and not data.empty) else None
    except: return None

# 4. LOOP DE ATUALIZAÇÃO (FORÇADO 2 SEGUNDOS)
placeholder = st.empty()

while True:
    with placeholder.container():
        s_df = fetch_data("BRL=X", premarket=False)
        d_df = fetch_data("DX-Y.NYB", premarket=True)
        e_df = fetch_data("EWZ", premarket=True)
        ...
        if s_df is not None:
            ...
            # Cálculos de Variação (DXY e EWZ com tratamento de pré-mercado)
