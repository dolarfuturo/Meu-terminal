...
while True:
    # Mercado NY abre 11:30 Brasília
    ...
    with placeholder.container():
        ...
        if s_df is not None and not s_df.empty:
            ...
            spread = d_v - e_v
            pari = v_aj * (1 + (spread/100))
            v_s = ((s_at - v_aj) / v_aj * 100)

            # --- TELAS ---
            # PARIDADE
            st.markdown(f'<div class="asset-row"><div class="name">PARIDADE</div><div class="price price-paridade">{pari:.4f}</div><div class="var {"pos" if spread >= 0 else "neg"}">{spread:+.2f}%</div></div>', unsafe_allow_html=True)
            ...
            st.markdown(f'<div class="frp-box"><div class="frp-item"><span>MAXIMA</span><span class="pos">{(v_ptax_m+(v_max/1000)):.4f}</span></div><div class="frp-item"><span>JUSTO </span><span class="blu">{(v_ptax_m+(v_jus/1000)):.4f}</span></div><div class="frp-item"><span>MINIMA </span><span class="neg">{(v_ptax_m+(v_min/1
