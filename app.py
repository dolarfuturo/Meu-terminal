...
if st.session_state.show_settings:
    ...
    # Apenas este botão salva e fecha
            ...
            with placeholder.container():
                st.markdown(f"""
                    ...
                    <div class="d-row"><div class="d-label">REF. INSTITUCIONAL</div><div style="display:flex; gap:15px;">
                        <div style="text-align:center"><small>MIN</small><br><span style="color:#ff3333">{(round((st.session_state.ptax+st.session_state.v22)*2000)/2000):.4f}</span></div>
                        <div style="text-align:center"><small>JUSTO</small><br><span style="color:#0066cc">{(round((st.session_state.ptax+st.session_state.v31)*2000)/2000):.4f}</span></div>
                        <div style="text-align:center"><small>MAX</small><br><span style="color:#00ff88">{(round((st.session_state.ptax+st.session_state.v42)*2000)/2000):.4f}</span></div>
                    </div></div>

                    <div class="d-row"><div class="d-label">REGIÕES DE CORREÇÃO</div>
                        <div style="display:flex; gap:40px;">
                            <div class="corr-box"><span class="val-11">{(st.session_state.ptax-0.0110):.4f}</span><span class="val-22">{(st.session_state.ptax-st.session_state.v22):.4f}</span></div>
                            <div class="corr-box"><span class="val-11">{(st.session_state.ptax+0.0110):.4f}</span><span class="val-22">{(st.session_state.ptax+st.session_state.v22):.4f}</span></div>
                        </div>
                    </div>
