...
def get_market_data(ticker):
    try:
        ...
        # Extrai apenas o último preço como um número float puro
        last = float(df['Close'].iloc[-1]) if not df.empty else prev
        return {"last": last, "prev": prev}
    except:
        return {"last": 0.0001, "prev": 0.0001}
...
while True:
    ...
    with placeholder.container():
        ...
        if not s_df.empty:
            ...
            # Cálculos das variações (Garante números puros)
            d_v = float(((d_m["last"] - d_m["prev"]) / d_m["prev"] * 100))
            e_v = float(((e_m["last"] - e_m["prev"]) / e_m["prev"] * 100))
            
            sprd = d_v - e_v
            pari = float(v_aj * (1 + (sprd/100)))
            v_s = float(((s_at - v_aj) / v_aj * 100))
            ...
            # --- BLOCO 5: EWZ E PRÉ-MERCADO ---
            if is_pre:
                # Preço estático de ontem
                ...
                # Variação real do Pré-Market agora
