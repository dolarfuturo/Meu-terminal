...
st.set_page_config(page_title="LIVE TERMINAL", layout="wide")

# Tempo de atualização
refresh_interval = 1 

# CSS Bloomberg - Otimizado para não piscar
st.markdown("""
    <style>
    ...
    .spread-box { border: 1px dashed #555555; padding: 10px; background-color: #111111; text-align: center; margin-bottom: 20px; }
    .price-text { font-size: 28px; font-family: 'Courier New'; font-weight: bold; }
    </style>
    """, unsafe_allow_html=True)

# Sidebar oculta
with st.sidebar.expander("⚙️ AJUSTAR PONTOS", expanded=False):
    v_min = st.number_input("Mínima", value=22.0)
    v_justo = st.number_input("Justo", value=31.0)
    v_max = st.number_input("Máxima", value=42.0)

def get_live_data(ticker):
    try:
        # Puxa apenas o último dia para máxima velocidade
        df = yf.download(ticker, period="1d", interval="1m", progress=False)
        return df
    except: return pd.DataFrame()

# Container que limpa e reconstrói a cada segundo
...
with placeholder.container():
    ...
    if not spot_df.empty:
        ...
        c2.metric("DXY", f"{dxy_df['Close'].iloc[-1]:.2f}", f"{v_dxy:.2f}%")
        c3.metric("EWZ", f"{ewz_df['Close'].iloc[-1]:.2f}", f"{v_ewz:.2f}%")

        # SPREAD COLORIDO
        spread = v_dxy - v_ewz
        cor = "#00FF00" if spread >= 0 else "#FF0000"
        st.markdown(f'<div class="spread-box">SPREAD DXY-EWZ: <span style="color:{cor}; font-size:24px; font-weight:bold;">{spread:.2f}%</span></div>', unsafe_allow_html=True)
