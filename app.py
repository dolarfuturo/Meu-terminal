...
<style>
    ...
    .pulse { animation: blinker 1s linear infinite; color: #ffff00; margin-right: 8px; font-size: 14px; }
...
# 5. MOTOR DE DATA (MELHORADO PARA REAL-TIME)
def get_clean_data(ticker):
    try:
        # Busca 1 dia com intervalo de 1m, sem cache
        df = yf.download(ticker, period="1d", interval="1m", progress=False, prepost=True)
        if df.empty:
            return {"last": 0.0, "prev": 0.0, "var": 0.0}
        
        last = float(df['Close'].iloc[-1])
        # Pega o fechamento anterior real do info do ticker
        t = yf.Ticker(ticker)
...
ui_area = st.empty()
while True:
    # Coleta de dados forçada a cada ciclo
    ...
    with ui_area.container():
        if s_m["last"] > 0:
            spot = s_m["last"]
            prev_close = s_m["prev"]
            cor_spot = "#00cc66" if s_m["var"] >= 0 else "#cc3333"
            ...
            diff = spot - justo
            clr = "#00aa55" if diff < -0.0015 else ("#aa3333" if diff > 0.0015 else "#aaaa00")
            arr = "▲ ▲ ▲ ▲ ▲" if diff < -0.0015 else ("▼ ▼ ▼ ▼ ▼" if diff > 0.0015 else "◄ ◄ ◄ ► ► ►")
            ...
            st.markdown(f'<div class="t-header"><div class="t-title"><span class="pulse">●</span>TERMINAL <span class="t-bold">DOLAR</span></div></div>', unsafe_allow_html=True)
            st.markdown(f'<div class="s-container" style="border-bottom: 2px solid {clr}77"><div class="s-text" style="color:#fff">SPOT {spot:.4f} <span style="color:{cor_spot}; margin-left:10px;">({s_m["var"]:+.2f}%)</span></div><div class="s-subtext">FECH. ANTERIOR: {prev_close:.4f}</div><div class="vies-indicator" style="color:{fut_clr}">{fut_seta}</div></div>', unsafe_allow_html=True)
            ...
            st.markdown(f'<div class="note-box"><div class="note-title">MORNING CALL & AGENDA</div><div class="note-content">{v_global["notas_mural"].replace("\\n", "<br>").replace("\n", "<br>")}</div></div>', unsafe_allow_html=True)
