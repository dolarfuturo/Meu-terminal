...
# 3. CSS - TERMINAL LIMPO (VARIÁVEIS NO TOPO COLADAS NOS ATIVOS)
st.markdown("""
    <style>
    ...
    * { font-family: 'Roboto Mono', monospace !important; text-transform: uppercase; }
    ...
    /* Esconde lixo visual do Streamlit */
    header, [data-testid="stHeader"], [data-testid="stToolbar"] { display: none !important; }
    .block-container { padding-top: 1rem !important; max-width: 800px !important; margin: auto; }

    /* TÍTULO NO TOPO */
    .main-title { font-size: 20px; font-weight: bold; color: #FFF; border-bottom: 1px solid #333; padding-bottom: 10px; margin-bottom: 15px; }

    /* INPUTS DE VARIÁVEIS COMPACTOS */
    ...
    .asset-row { display: flex; gap: 15px; margin-bottom: 8px; align-items: center; }
    ...
    /* PONTOS VERTICAIS (FRP) */
    .frp-box { margin-left: 165px; margin-top: -5px; margin-bottom: 15px; display: flex; flex-direction: column; gap: 2px; }
    ...
    .trava-orange { color: #FF8C00 !important; font-size: 18px; margin-top: 15px; font-weight: bold; border-top: 1px solid #333; padding-top: 10px; }
    </style>
    """, unsafe_allow_html=True)

# 4. TÍTULO E PAINEL DE VARIÁVEIS (AGORA NO TOPO PRINCIPAL)
st.markdown('<div class="main-title">TERMINAL DE CÂMBIO</div>', unsafe_allow_html=True)

# Colunas de inputs para caber tudo em uma linha
...
with c3: v_max = st.number_input("MAX PTS", value=42.0)
with c4: v_jus = st.number_input("JUS PTS", value=31.0)
with c5: v_min = st.number_input("MIN PTS", value=22.0)
...
while True:
    with placeholder.container():
        ...
        if s_df is not None:
            # Correção do TypeError: garante que pegamos apenas o valor numérico
