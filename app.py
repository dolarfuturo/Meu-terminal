...
while True:
    ...
    with placeholder.container():
        ...
        if s_df is not None and not s_df.empty:
            ...
            st.markdown(f'<div class="asset-row"><div class="name">PARIDADE</div><div class="price price-paridade">{pari:.4f}</div><div class="var {c_pari}">{sprd:+.2f}%</div></div>', unsafe_allow_html=True)
            ...
            # DXY INDEX
            c_d = "pos" if d_v >= 0 else "neg"
            st.markdown(f'<div class="asset-row"><div class="name">DXY INDEX</div><div class="price">{d_m["last"]:.2f}</div><div class="var {c_d}">{d_v:+.2f}%</div></div>', unsafe_allow_html=True)
            ...
            if is_pre:
                ...
                c_e = "pos" if e_v >= 0 else "neg"
                st.markdown(f'<div class="pre-row"><div class="pre-name">∟ PRE-MARKET</div><div class="pre-price">{e_m["last"]:.2f}</div><div class="pre-var {c_e}">{e_v:+.2f}%</div></div>', unsafe_allow_html=True)
            else:
                c_e = "pos" if e_v >=
