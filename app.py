...
if st.session_state.show_settings:
    ...
    with caixa_var:
        ...
        st.session_state.v31 = st.number_input("Variação 31 pts", value=st.session_state.v31, format="%.4f")
        st.session_state.v42 = st.number_input("Variação 42 pts", value=st.session_state.v42, format="%.4f")
        st.session_state.txt_topo = st.text_input("Texto Livre", value=st.session_state.txt_topo)
    if st.button("SALVAR"):
...
while True:
    ...
    if s_p > 0:
        ...
        if diff_gauge > 0.10: alert, clr = "PRECIFICAÇÃO DE ALTA", "#00ff88"
        elif diff_gauge < -0.10: alert, clr = "PRECIFICAÇÃO DE BAIXA", "#ff3333"
        ...
        with main_area.container():
            ...
            tk = (
                ...
                f"<span class='tk-item'>EWZ: {ew_p:.2f} <span style='color:{col(ew_v)}'>({ew_v:+.2f}%)</span></span>"
                f"<span class='tk-item'>EURUSD: {eu_p:.4f} <span style='color:{col(eu_v)}'>({eu_v:+.2f}%)</span></span>"
