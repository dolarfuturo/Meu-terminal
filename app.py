...
# 2. FUNÇÕES DE TRAVA (CALLBACKS)
def update_val():
    # Esta função força a sincronização manual dos dados
    for k in ['ajuste', 'ptax', 'ref', 'v22', 'v31', 'v42', 'txt_topo']:
        if f"inp_{k}" in st.session_state:
            st.session_state[k] = st.session_state[f"inp_{k}"]
...
<style>
    ...
    .stNumberInput div div input { background-color: #111 !important; color: #fff !important; border: 1px solid #444 !important; font-size: 16px !important; font-weight: bold !important; }
    .stButton button { background-color: #111 !important; color: #888 !important; border: 1px solid #333 !important; }
    ...
    .gauge-bg { position: absolute; top: 0; left: 0; width: 140px; height: 140px; border-radius: 50%; background: conic-gradient(#ff3333 0deg 60deg, #ffff00 600deg 120deg, #00ff88 120deg 180deg, #000 180deg); transform: rotate(-90deg); }
    ...
    .id-tag { color: #00ff88; font-size: 12px; font-weight: 900; margin-bottom: -15px; }
</style>
""", unsafe_allow_html=True)

# 5. MENU DE CONFIGURAÇÕES (SET)
if st.button("⚙️ SET / TRAVAR VALORES"):
    st.session_state.show_settings = not st.session_state.show_settings
    st.rerun()

if st.session_state.show_settings:
    st.markdown("<h3 style='text-align:center; color:orange;'>CONFIGURAÇÕES TÉCNICAS</h3>", unsafe_allow_html=True)
    c1, c2 = st.columns(2)
    with c1:
        st.markdown("<p class='id-tag'>[ IDENTIFICAÇÃO: AJUSTE ]</p>", unsafe_allow_html=True)
        st.number_input("", value=st.session_state.ajuste, format="%.4f", key="inp_ajuste", on_change=update_val)
        
        st.markdown("<p class='id-tag'>[ IDENTIFICAÇÃO: PTAX ]</p>", unsafe_allow_html=True)
        st.number_input("", value=st.session_state.ptax, format="%.4f", key="inp_ptax", on_change=update_val)
        
        st.markdown("<p class='id-tag'>[ REF. INSTITUCIONAL ]</p>", unsafe_allow_html=True)
        st.number_input("", value=st.session_state.ref, format="%.4f", key="inp_ref", on_change=update_val)
    with c2:
        st.write("VARIÁVEIS DE PONTOS")
        st.number_input("VAR 22", value=st.session_state.v22, format="%.4f", key="inp_v22", on_change=update_val)
        st.number_input("VAR 31", value=st.session_state.v31, format="%.4f", key="inp_v31", on_change=update_val)
        st.number_input("VAR 42", value=st.session_state.v42, format="%.4f", key="inp_v42", on_change=update_val)
    
    st.text_input("FRASE TOPO", value=st.session_state.txt_topo, key="inp_txt_topo", on_change=update_val)
    st.info("Os valores acima são salvos AUTOMATICAMENTE ao digitar e apertar Enter.")
    st.stop()
