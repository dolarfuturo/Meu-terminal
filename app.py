import streamlit as st
import yfinance as yf
import pandas as pd
from datetime import datetime

# 1. CONFIGURAÇÃO DO TERMINAL
st.set_page_config(page_title="TERMINAL", layout="wide")

# 2. ESTILO CSS (DARK MODE PROFISSIONAL)
st.markdown("""
<style>
    @import url('https://fonts.googleapis.com/css2?family=Roboto+Mono:wght@400;700&display=swap');
    * { font-family: 'Roboto Mono', monospace !important; text-transform: uppercase; }
    .stApp { background-color: #000000; color: #FFFFFF; }
    header, [data-testid="stHeader"], [data-testid="stToolbar"] { display: none !important; }
    .block-container { padding-top: 1rem !important; max-width: 600px !important; margin: auto; }
    .main-title { font-size: 20px; font-weight: bold; border-bottom: 1px solid #333; padding-bottom: 5px; margin-bottom: 15px; color: #00FF00; }
    
    .asset-row { display: flex; justify-content: space-between; align-items: center; margin-bottom: 8px; padding: 10px; border-bottom: 1px solid #111; background-color: #050505; }
    .name { font-size: 14px; color: #888; font-weight: bold; width: 40%; }
    .price { font-size: 18px; font-weight: bold; color: #FFF; width: 30%; text-align: right; }
    .var { font-size: 16px; font-weight: bold; width: 30%; text-align: right; }

    .pos { color: #00FF00 !important; }
    .neg { color: #FF0000 !important; }
    .trava-orange { color: #FF8C00 !important; font-size: 16px; margin-top: 20px; font-weight: bold; border-top: 1px solid #333; padding-top: 10px; }
</style>
""", unsafe_allow_html=True)

# 3. FUNÇÃO DE DADOS COM CACHE E TRATAMENTO DE ERRO
@st.cache_data(ttl=30)
def get_crypto_data(ticker):
    try:
        data = yf.Ticker(ticker)
        df = data.history(period="2d")
        if df.empty or len(df) < 2:
            return 0.0, 0.0
        price = float(df['Close'].iloc[-1])
        prev = float(df['Close'].iloc[-2])
        var = ((price - prev) / prev) * 100
        return price, var
    except:
        return 0.0, 0.0

st.markdown('<div class="main-title">TERMINAL DE CÂMBIO</div>', unsafe_allow_html=True)

# 4. INPUTS (Removi o popover para evitar o bug de ícone "arrow_right")
col_inp1, col_inp2 = st.columns(2)
with col_inp1:
    v_aj = st.number_input("AJUSTE", value=5.3900, format="%.4f")
with col_inp2:
    v_ptax_m = st.number_input("PTAX", value=5.3850, format="%.4f")

st.markdown("---")

# 5. LISTA DE ATIVOS
ativos = {
    "DÓLAR B3": "USDBRL=X",
    "DXY INDEX": "DX-Y.NYB",
    "S&P 500 FUT": "ES=F"
}

for nome, ticker in ativos.items():
    valor, variacao = get_crypto_data(ticker)
    
    cor_classe = "pos" if variacao >= 0 else "neg"
    
    # Formatação segura para evitar o ValueError da linha 67
    exibir_valor = f"{valor:.3f}" if valor > 0 else "CARREGANDO..."
    exibir_var = f"{variacao:+.2f}%" if valor > 0 else "0.00%"

    st.markdown(f"""
    <div class="asset-row">
        <div class="name">{nome}</div>
        <div class="price">{exibir_valor}</div>
        <div class="var {cor_classe}">{exibir_var}</div>
    </div>
    """, unsafe_allow_html=True)

# 6. RODAPÉ
st.markdown(f'<div class="trava-orange">REF. AJUSTE: {v_aj:.4f}</div>', unsafe_allow_html=True)

if st.button("🔄 ATUALIZAR"):
    st.rerun()
