...
# CSS: PAINEL ROBUSTO, FONTES CHEIAS E RODAPÉ ESTILIZADO
st.markdown("""
    <style>
    .stApp { background-color: #0b0e11 !important; color: #ffffff !important; }
    
    /* Título Extra Bold / Letra Cheia */
    ...
    .clock-label { color: #848e9c; font-size: 9px; text-transform: uppercase; }

    /* BORDAS DO PAINEL (Reforçadas) */
    .grid-container { 
    ...
    .tk-name { color: #ffffff !important; font-weight: 900; }
    .tk-up { color: #00ff88; font-weight: 900; }
    .tk-down { color: #ff4d4d; font-weight: 900; }
    
    /* Estilo Botão ADM */
    .stButton>button { width: 100%; background-color: #00f2ff !important; color: #000 !important; font-weight: 900 !important; border-radius: 5px !important; }
    </style>
...
# --- PAINEL ADM COM BOTÃO ---
with st.expander("⚙️ PAINEL ADM"):
    c1, c2 = st.columns(2)
    with c1: adm_val = st.text_input("PAINEL ADM:", "5,4000")
...
with m_col:
    ...
    st.markdown('<p style="color:#00f2ff; font-weight:900; font-size:14px;">MONITORAMENTO DA GRADE PRINCIPAL</p>', unsafe_allow_html=True)
    ...
    for a in ativos:
        t_html += f"<tr><td><span class='asset-tag'>{a}</span></td><td>5.4000</td><td>5.0000</td><td>5.0000</td><td>5.0000</td><td>0.000</td><td>0.000</td></tr>"
    st.markdown(t_html + "</table></div>", unsafe_allow_html=True)

with s_col:
    st.markdown('<div class="grid-container">', unsafe_allow_html=True)
    st.markdown('<p style="color:#ffcc00; font-weight:900; font-size:16px; text-align:center;">PAINEL DE CONTROLE CÁLCULOS</p>', unsafe_allow_html=True)
    ...
    # Baixas (Incluindo o -2,66% solicitado)
...
st.markdown(f"""
    <div class="footer-ticker">
        <div class="ticker-move">
            ...
            <span class="tk-name">SPOT</span> <span class="tk-name">● 0,00%</span> | 
            <span class="tk-name">GBPUSD</span> <span class="tk-up">▲ 1,02%</span> |
            <span class="tk-name">XAUUSD</span> <span class="tk-down">▼ -0,05%</span>
        </div>
    </div>
    """, unsafe_allow_html=True)
