...
def calcular_eixo_automatico():
    try:
        ...
        data_ref = datas[-1] if agora.hour >= 18 else datas[-1]
...
def calcular_k97_total(eixo_ewz, p_ewz_atual, max_ewz, min_ewz, eixo_dol):
    try:
        ...
        return {
            ...
            "max": alvo_max, "p75_up": (e_dol + (alvo_max - e_dol)*0.75), 
            "p50_up": (eixo_dol + alvo_max) / 2, 
            "p25_up": (e_dol + (alvo_max - e_dol)*0.25),
            "min": alvo_min, "p75_down": (e_dol + (alvo_min - e_dol)*0.75), 
            "p50_down": (eixo_dol + alvo_min) / 2, 
            "p25_down": (e_dol + (alvo_min - e_dol)*0.25)
        }
    except: return None
...
if data:
    ...
    if res:
        ...
        with c1:
            ...
            # EWZ VIVO e Máxima/Mínima Realtime
            st.metric("EWZ VIVO", f"{data['at']:.2f}", delta=f"{res['v_atual']:+.2f}%")
            st.markdown(f"""
            <div class="realtime-box">
                MAX REAL: <span style="color:#ff4d4d">{data['mx_real']:.2f}</span><br>
                MIN REAL: <span style="color:#00ff88">{data['mn_real']:.2f}</span>
            </div>
