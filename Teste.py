import streamlit as st
import yfinance as yf
import time
from datetime import datetime

# Configuração para Tablet
st.set_page_config(page_title="K97 - EIXO AUTO 10:30-17:00", layout="wide")

# --- FUNÇÃO QUE CALCULA O EIXO (MÁX+MÍN)/2 DO PREGÃO ANTERIOR ---
@st.cache_data(ttl=3600)
def calcular_eixo_anterior():
    try:
        t = yf.Ticker("EWZ")
        # Puxa histórico para achar o último dia útil
        df = t.history(period="5d", interval="15m")
        if df.empty: return 37.85, 0, 0
        
        # Pega a data do dia anterior ao de hoje
        datas = df.index.normalize().unique()
        data_ontem = datas[-2] 
        
        # Filtra o dia e o horário das 10:30 às 17:00 (Brasília) -> 09:30 às 16:00 (NY)
        df_ontem = df.loc[data_ontem.strftime('%Y-%m-%d')]
        df_periodo = df_ontem.between_time('09:30', '16:00')
        
        max_ontem = df_periodo['High'].max()
        min_ontem = df_periodo['Low'].min()
        eixo = (max_ontem + min_ontem) / 2
        return eixo, max_ontem, min_ontem
    except:
        return 37.85, 0, 0

@st.cache_data(ttl=2)
def fetch_data():
    try:
        t = yf.Ticker("EWZ")
        df = t.history(period="1d", interval="1m", prepost=True)
        return {"at": df['Close'].iloc[-1], "mx": df['High'].max(), "mn": df['Low'].min()} if not df.empty else None
    except: return None

# --- MOTOR DE CÁLCULO K97 ---
def calcular_k97_total(eixo_ewz, p_ewz_atual, max_ewz, min_ewz, eixo_dol):
    try:
        var_atual = ((eixo_ewz / p_ewz_atual) - 1) * 100 / 2
        dolar_vivo = eixo_dol * (1 + (var_atual / 100))
        var_fraja = ((eixo_ewz / p_ewz_atual) - 1) * 100 / 3.6
        dolar_fraja = eixo_dol * (1 + (var_fraja / 100))
        
        var_neg = ((eixo_ewz / max_ewz) - 1) * 100 / 2
        var_pos = ((eixo_ewz / min_ewz) - 1) * 100 / 2
        alvo_max = eixo_dol * (1 + (var_pos / 100))
        alvo_min = eixo_dol * (1 + (var_neg / 100))
        
        return {
            "vivo": dolar_vivo, "fraja": dolar_fraja, "v_atual": var_atual,
            "max": alvo_max, "p50_up": (eixo_dol + alvo_max) / 2,
            "min": alvo_min, "p50_down": (eixo_dol + alvo_min) / 2
        }
    except: return None

# --- ESTILO ---
st.markdown("""<style>
    .stApp { background-color: #0b0e11 !important; color: #ffffff !important; }
    .vivo-box { background: #161b22; border: 2px solid #ffcc00; padding: 15px; text-align: center; border-radius: 8px; }
    .label-k97 { color: #00f2ff; font-size: 13px; font-weight: bold; }
    .valor-vivo { font-size: 42px; font-family: 'Arial Black'; color: #ffcc00; }
</style>""", unsafe_allow_html=True)

# Execução
eixo_auto, mx_ant, mn_ant = calcular_eixo_anterior()
data = fetch_data()

with st.sidebar:
    st.header("⚙️ AJUSTE K97")
    # O EIXO JÁ APARECE AQUI CALCULADO
    e_ewz = st.number_input("EIXO EWZ (AUTO):", value=float(eixo_auto), format="%.2f")
    e_dol = st.number_input("EIXO DOLFUT:", value=5219.50, format="%.2f")
    st.write(f"Ref Ontem: Máx {mx_ant:.2f} / Mín {mn_ant:.2f}")

if data:
    res = calcular_k97_total(e_ewz, data["at"], data["mx"], data["mn"], e_dol)
    if res:
        c1, c2 = st.columns([1, 1.2])
        with c1:
            st.markdown(f'<div class="vivo-box"><div class="label-k97">SINTÉTICO (2.0)</div><div class="valor-vivo">{res["vivo"]:.2f}</div></div>', unsafe_allow_html=True)
            st.metric("SINTÉTICO (3.6)", f"{res['fraja']:.2f}")
            st.metric("EWZ", f"{data['at']:.2f}", delta=f"{res['v_atual']:+.2f}%")
        with c2:
            st.error(f"MÁXIMA: {res['max']:.2f}")
            st.warning(f"50% UP: {res['p50_up']:.2f}")
            st.info(f"EIXO: {e_dol:.2f}")
            st.success(f"50% DN: {res['p50_down']:.2f}")
            st.success(f"MÍNIMA: {res['min']:.2f}")

time.sleep(2)
st.rerun()
