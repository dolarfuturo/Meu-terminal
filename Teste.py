...
def calcular_dolfut_k97(eixo_ewz_ontem, preco_ewz_atual, eixo_dolfut_manual):
    try:
        # (EIXO FILTRADO ONTEM / PREÇO AGORA - 1) * 100 / 2
...
# --- CAPTURA DE DADOS COM FILTRO DE HORÁRIO (11:30 - 18:00) ---
@st.cache_data(ttl=5)
def fetch_ewz_data():
    try:
        t = yf.Ticker("EWZ")
        # Pega 5 dias para garantir que pegamos a última sexta caso hoje seja segunda
        df = t.history(period="5d", interval="1m", prepost=True)
        
        if not df.empty:
            dias = df.index.normalize().unique()
            # O dia anterior de negociação é o penúltimo no dataframe (dias[-2])
            dia_anterior = dias[-2]
            df_ontem = df[df.index.normalize() == dia_anterior]
            
            # FILTRO EXATO: 11:30 às 18:00 (Horário de Brasília/Yahoo costuma ser ET, ajuste manual se necessário)
            # Aqui filtramos o miolo do pregão regular de ontem
            df_regular = df_ontem.between_time('11:30', '18:00')
            
            if df_regular.empty: # Fallback caso o filtro falhe por fuso
                df_regular = df_ontem
                
            eixo_filtrado = (df_regular['High'].max() + df_regular['Low'].min()) / 2
            preco_atual = df['Close'].iloc[-1]
            
            return {
                "price": preco_atual,
                "eixo_ontem": eixo_filtrado,
...
if market:
    ...
    with col1:
        ...
        st.markdown('<div class="metric-label">EWZ (AGORA)</div>', unsafe_allow_html=True)
        st.markdown(f'<div class="metric-val">{market["price"]:.2f}</div>', unsafe_allow_html=True)
        st.markdown(f'<div style="color: #848e9c; font-size: 14px;">Eixo ({market["data_eixo"]} 11:30-18h): {market["eixo_ontem"]:.2f}</div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)

    with col2:
        st.markdown('<div class="frame-box">', unsafe_allow_html=True)
        st.markdown('<div class="metric-label">DOLFUT SINTÉTICO</div>', unsafe_allow_html=True)
    ...
    st.error("Calculando Eixo 11:30-18:00 do dia anterior...")
