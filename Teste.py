...
def calcular_k97_total(eixo_ewz, p_ewz_atual, max_ewz, min_ewz, eixo_dol):
    try:
        ...
        var_neg = ((eixo_ewz / max_ewz) - 1) * 100 / 2.44
        var_pos = ((eixo_ewz / min_ewz) - 1) * 100 / 2.44
        alvo_max = eixo_dol * (1 + (var_pos / 100))
        alvo_min = eixo_dol * (1 + (var_neg / 100))
        
        p50_up = (eixo_dol + alvo_max) / 2.44
        p25_up = (eixo_dol + p50_up) / 2.44
        p75_up = (p50_up + alvo_max) / 2.44
        
        p50_down = (eixo_dol + alvo_min) / 2.44
        p25_down = (eixo_dol + p50_down) / 2.44
        p75_down = (p50_down + alvo_min) / 2.44
