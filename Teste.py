...
import hashlib
import uuid
from streamlit_gsheets import GSheetsConnection # Biblioteca para escrita

# --- SEU SETUP E CSS MANTIDOS EXATAMENTE IGUAIS ---

def verificar_acesso():
    # Conexão oficial para ESCRITA e LEITURA
    ...
    if "autenticado" not in st.session_state:
        ...
        if btn and chave:
            ...
            try:
                # Lendo a planilha via conexão oficial
                df = conn.read(ttl=0) # ttl=0 garante que lê o dado mais atual
                hash_t = hashlib.sha256(chave.encode()).hexdigest()
                df.columns = df.columns.str.strip()
                
                # Procura o usuário
                idx = df.index[(df['HASH_SENHA'].astype(str).str.strip() == hash_t) & (df['STATUS'].str.strip() == 'ATIVO')]
                
                if not idx.empty:
                    novo_id_sessao = str(uuid.uuid4())[:8]
                    row_idx = idx[0] + 2 # Ajuste para o índice do Google Sheets
                    
                    # --- BLOQUEIO AUTOMÁTICO (ESCRITA) ---
                    # Grava o ID único na Coluna E (SESSAO)
                    conn.update(worksheet="Página1", data=[[novo_id_sessao]], range=f"E{row_idx}")
                    
                    st.session_state["autenticado"] = True
                    st.session_state["usuario"] = df.loc[idx[0], 'CLIENTE']
                    st.session_state["session_id"] = novo_id_sessao
...
# --- NO SEU LOOP PRINCIPAL (while True) ---
# Adicione esta trava no topo do seu loop:
while True:
    try:
        if st.session_state.get("role") == "user":
            # Verifica se o ID na planilha ainda é o mesmo deste terminal
            conn = st.connection("gsheets", type=GSheetsConnection)
            ...
            if not user_row.empty:
                id_na_planilha = str(user_row.iloc[0]['SESSAO']).strip()
                if id_na_planilha != st.session_state["session_id"]:
                    st.error("⚠️ CONTA CONECTADA EM OUTRO LOCAL. ESTE TERMINAL FOI BLOQUEADO.")
                    st.stop()
        
        # --- RESTO DO CÓDIGO (MOEDAS E LAYOUT) ---
