...
while True:
    ...
    <div class="top-header-fixed">
        ...
        <div class="header-grid">
            ...
            <div class="h-col" style="color:#FFA500;">PRÓX. TOPO</div>
            <div class="h-col" style="color:#FFFF00;">DECISÃO</div>
            <div class="h-col" style="color:#00CED1;">RESPIRO</div>
            <div class="h-col" style="color:#FFA500;">PRÓX. FUNDO</div>
...
for t, info in COINS_CONFIG.items():
    ...
    # Parâmetros de Volatilidade Âncora Vision
    p_exausto_t, p_topo, p_decisao, p_respiro = 1.0122, 1.0082, 1.0061, 1.0040
    p_exausto_f, p_fundo, p_respiro_b = 0.9878, 0.9939, 0.9960
    
    var_escada = ((price / mp) - 1) * 100
    if var_escada >= 1.22: 
        st.session_state[f'mp_{t}'] = mp * 1.0122
    elif var_escada <= -1.22: 
        st.session_state[f'mp_{t}'] = mp * 0.9878
    
    var_reset = ((price / rv) - 1) * 100
    cor_v, seta_v = ("#00FF00", "▲") if var_reset >= 0 else ("#FF4444", "▼")
    
    blink_t = "animation: blink 0.4s infinite;" if (var_escada >= 1.10) else ""
    blink_f = "animation: blink 0.4s infinite;" if (var_escada <= -1.10) else ""
    ...
    st.markdown(f"""
        <div class="row-container">
            ...
            <div class="w-col" style="color:#FF4444; {blink_t}">{f"{(mp * p_exausto_t):,.{info['dec']}f}"}</div>
            <div class="w-col" style="color:#FFA500;">{f"{(mp * p_topo):,.{info['dec']}f}"}</div>
            <div class="w-col" style="color:#FFFF00;">{f"{(mp * p_decisao):,.{info['dec']}f}"}</div>
            <div class="w-col" style="color:#00CED1;">{f"{(mp * p_respiro):,.{info['dec']}f}"}</div>
            <div class="w-col" style="color:#FFA500;">{f"{(mp * p_fundo):,.{info['dec']}f}"}</div>
            <div class="w-col" style="color:#00FF00; {blink_f}">{f"{(mp * p_exausto_f):,.{info['dec']}f}"}</div>
        </div>
        <div class="vision-block">
            ...
            <div class="v-item">
                <div style="color:#666; font-size:8px;">ÂNCOVISION</div>
