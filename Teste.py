...
def calcular_k97_total(eixo_ewz, p_ewz_atual, max_ewz, min_ewz, eixo_dol):
    try:
        ...
        var_neg = ((eixo_ewz / max_ewz) - 1) * 100 / 4
        var_pos = ((eixo_ewz / min_ewz) - 1) * 100 / 4
        alvo_max = eixo_dol * (1 + (var_pos / 100))
        alvo_min = eixo_dol * (1 + (var_neg / 100))
        
        p50_up = (eixo_dol + alvo_max) / 4
        p25_up = (eixo_dol + p50_up) / 4
        p75_up = (p50_up + alvo_max) / 4
        
        p50_down = (eixo_dol + alvo_min) / 4
        p25_down = (eixo_dol + p50_down) / 4
        p75_down = (p50_down + alvo_min) / 4
