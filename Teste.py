...
st.markdown("""
    ...
    /* Painel de Cálculos Operacionais - FONTE AJUSTADA */
    .calc-row { 
        display: flex; 
...
# FUNÇÃO PARA TROCAR PONTO POR VÍRGULA COM PRECISÃO DE 4 CASAS
def fmt_v(val):
    try:
        if isinstance(val, (int, float)):
            # Mantém a precisão original dependendo do ativo
            if val > 1000: return f"{val:.1f}".replace(".", ",")
            if val < 2: return f"{val:.4f}".replace(".", ",")
