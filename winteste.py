...
def calcular_k97_total(eixo_ewz, p_ewz_atual, max_ewz, min_ewz, eixo_win):
    try:
        ...
        dolar_medio = eixo_win * (1 + (var_medio / 100)) 
        
        v_neg = ((eixo_ewz / max_ewz) - 1) * 100 / 3
        v_pos = ((eixo_ewz / min_ewz) - 1) * 100 / 3
        alvo_max = eixo_win * (1 + (v_pos / 100))
        alvo_min = eixo_win * (1 + (v_neg / 100))
        
        return {
            "vivo": dolar_vivo, "fraja": dolar_fraja, "medio": dolar_medio, 
            "v_atual": var_atual, "ewz_med": ewz_medio_dia,
            "max": alvo_max, "p75_up": (eixo_win + (alvo_max - eixo_win)*0.75), 
            "p50_up": (eixo_win + alvo_max) / 2, 
            "p25_up": (eixo_win + (alvo_max - eixo_dol)*0.25),
            "min": alvo_min, "p75_down": (eixo_win + (alvo_min - eixo_win)*0.75), 
            "p50_down": (eixo_dol + alvo_min) / 2, 
            "p25_down": (eixo_win + (alvo_min - eixo_win)*0.25)
        }
    except: return None
...
with st.sidebar:
    ...
    e_dol = st.number_input("EIXO WINFUT:", value=5219.50, format="%.2f")
    st.markdown("---")
