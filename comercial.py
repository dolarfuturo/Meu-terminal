...
<style>
    ...
    * { font-family: 'Roboto Mono', monospace !important; text-transform: uppercase; }
    ...
    /* TÍTULOS EM BRANCO PARA NITIDEZ */
    .section-title { font-size: 11px; color: #FFFFFF; letter-spacing: 3px; margin: 25px 0 10px 0; text-align: center; font-weight: bold; }

    /* LINHA SUPERIOR: PARIDADE E EQUILIBRIO */
    .dual-container { display: flex; justify-content: space-between; gap: 15px; margin-bottom: 5px; }
    .info-box { background: #080808; padding: 20px 10px; border-radius: 4px; border: 1px solid #1A1A1A; text-align: center; width: 50%; }
...
# 3. ENTRADAS (CLIQUE NO ÍCONE NO TOPO PARA CONFIGURAR)
with st.popover("⚙️ CONFIGURAÇÕES"):
    v_ajuste = st.number_input("AJUSTE ANTERIOR", value=5.4000, format="%.4f")
    st.session_state.ptax_dia = st.number_input("PTAX DO DIA", value=st.session_state.ptax_dia, format="%.4f")
...
while True:
    ...
    with placeholder.container():
        ...
        if st.session_state.ref_institucional > 0:
            if spot >= t_max: 
                st.markdown('<div class="tag-modern tag-caro">DOLAR CARO</div>', unsafe_allow_html=True)
            elif spot <= t_min: 
                st.markdown('<div class="tag-modern tag-barato">DOLAR BARATO</div>', unsafe_allow_html=True)
        st.markdown('</div>', unsafe_allow_html=True)

        # 1. PARIDADE E PONTO DE EQUILÍBRIO (LADO A LADO)
        st.markdown(f"""
        <div class="dual-container">
            <div style="width:48%;">
                <div class="section-title">PARIDADE</div>
                <div class="info-box" style="width:100%; border-bottom: 2px solid #FFB900;">
                    <div class="pari-val">{paridade:.4f}</div>
                </div>
            </div>
            <div style="width:48%;">
                <div class="section-title">PONTO
