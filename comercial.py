...
if st.session_state.user_type == "ADM":
    with st.sidebar:
        ...
        v_global["ref"] = st.number_input("REFERÊNCIA:", value=v_global["ref"], format="%.4f", step=0.0001)
        st.success("Sincronizado para todos.")
else:
    # CLIENTE NÃO TEM SIDEBAR
    st.markdown("<style>[data-testid='stSidebar'] { display: none !important; }</style>", unsafe_allow_html=True)

# 5. CSS REVISADO (MOSTRA A SETA APENAS PARA O ADM)
st.markdown("""
<style>
    @import url('https://fonts.googleapis.com/css2?family=Chakra+Petch:wght@400;700&family=Orbitron:wght@400;900&display=swap');
    
    /* ESCONDE ELEMENTOS CHATOS (FORK, GITHUB, ETC) */
    .stAppDeployButton, [data-testid="stToolbar"], footer { display: none !important; }
    
    /* MOSTRA O HEADER (ONDE FICA A ENGRENAGEM/SETA) APENAS PARA ADM */
    header { 
...
while True:
    ...
    if m:
        ...
        with main_ui.container():
            ...
            # LINHAS UTILIZANDO v_global PARA SINCRONIZAÇÃO
