...
# Conexão com a planilha (Nuvem)
conn = st.connection("gsheets", type=GSheetsConnection)

def carregar_valores_mestre():
    try:
        # Lê a planilha onde você salva os ajustes (Células A2 e B2)
        df = conn.read(worksheet="Sheet1", ttl="2s")
        ajuste = float(df.iloc[0, 0])
        referencial = float(df.iloc[0, 1])
        return ajuste, referencial
    except:
        return 5.4000, 5.3700 # Valores padrão caso falhe
...
# 3. CONTROLES DE ADMIN (OCULTOS)
# Carrega os valores da nuvem
v_ajuste_cloud, ref_base_cloud = carregar_valores_mestre()

with st.sidebar:
    st.title("SALA DE COMANDO")
    senha = st.text_input("CHAVE MESTRA", type="password")
    if senha == "1234": # COLOQUE SUA SENHA AQUI
        st.subheader("AJUSTE MANUAL")
        novo_ajuste = st.number_input("AJUSTE ANTERIOR", value=v_ajuste_cloud, format="%.4f")
        novo_ref = st.number_input("REFERENCIAL", value=ref_base_cloud, format="%.4f")
        if st.button("ATUALIZAR TERMINAL PARA TODOS"):
            df_up = pd.DataFrame({"ajuste": [novo_ajuste], "referencial": [novo_ref]})
...
# LOOP DE ATUALIZAÇÃO
while True:
    # Busca valores atualizados por VOCÊ na planilha
    v_ajuste, ref = carregar_valores_mestre()
