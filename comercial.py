...
if 'msg' not in st.session_state: st.session_state.msg = "AGUARDANDO ABERTURA"
...
# 3. CSS SEGURO
css = """<style>
@import url('https://fonts.googleapis.com/css2?family=Roboto+Mono:wght@700;800&display=swap');
* { font-family: 'Roboto Mono', monospace !important; text-transform: uppercase; }
.stApp { background: #000; color: #fff; }
header, [data-testid="stHeader"] { display: none !important; }
.block-container { padding: 1rem !important; max-width: 900px !important; margin: auto; }
[data-testid="stPopover"] { position: fixed; top: 0; right: 0; opacity: 0; z-index: 99; }
.row { display: flex; justify-content: space-between; align-items: center; padding: 15px 0; border-bottom: 1px solid #111; }
.lbl { font-size: 11px; font-weight: 800; color: #fff; }
.val { font-size: 32px; font-weight: 800; }
.grid { display: flex; gap: 20px; }
.s-lbl { font-size: 10px; color: #fff; font-weight: 800; display: block; margin-bottom: 2px; }
.s-val { font-size: 22px; font-weight: 800; }
.bar-bg { width: 100%; height: 4px; background: #111; margin-top: 5px; }
.bar-f { height: 100%; transition: 0.5s; }
.badge { font-size: 9px; padding: 2px 5px; border-radius: 2px; margin-left: 5px; }
.ft { position: fixed; bottom: 0; left: 0; width: 100%; background: #080808; padding: 10px; border-top: 1px solid #222; font-size: 10px; text-align: center; }
</style>"""
...
def get_data():
    try:
        t = ["BRL=X", "DX-Y.NYB", "EWZ", "EURUSD=X", "USDJPY=X"]
        d = {}
        for s in t:
...
while True:
    ...
    if m:
        s_p = m["BRL=X"]["p"]
        r_f = st.session_state.v_rf
        jst = (round((s_p + 0.0310) * 2000) / 2000)
        p_v = max(0, min(100, (spr + 2) * 25))
        p_c = "#f44" if spr > 0 else "#0f8"
        dif = s_p - jst
        ...
        with scr.container():
            ...
            st.markdown(f'<div class="row"><div class="lbl">SPOT <span class="badge" style="background:{b_c};">{b_t}</span></div><div class="val">{s_p:.4f}</div></div>', unsafe_allow_html=True)
            ...
            st.markdown(f'<div class="ft"><div style="color:#fb0; margin-bottom:5px;">⚠️ {st.session_state.msg}</div>LIVE | DXY {
