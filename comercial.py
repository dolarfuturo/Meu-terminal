...
# 2. ESTADO GLOBAL (Adicionado Fraldão)
...
v_global = st.session_state.v_global
...
while True:
    ...
    if s_m["last"] > 0:
        ...
        spr = d_m["var"] - e_m["var"]
        pari_val = abs(v_global["ajuste"] * (1 + (spr/100)))
            ...
            if st.session_state.user_type == "ADM":
                with st.expander("PAINEL ADM"):
                    with st.form("adm_panel"):
                        ...
                        v_global["fraldao"] = c3.number_input("FRALDÃO (PTS)", value=v_global["fraldao"])
