...
def get_global_vars():
    return {
        "ajuste": 5.4000, 
        "ref": 5.4000,
        "fraldao": 15.0, # Diferença de pontos para o Futuro
...
<style>
    ...
    /* ESTILOS NOVOS */
    .v-futuro { font-size: 16px; color: #444; font-family: 'Chakra Petch'; font-weight: 700; }
...
while True:
    ...
    if s_m["last"] > 0:
        spot = s_m["last"]
        # PREÇO FUTURO (SINTÉTICO)
        dol_futuro = spot + (v_global.get("fraldao", 15.0) / 1000)
        
        spr = d_m["var"] - e_m["var"]
        equilibrio = round((v_global["ref"] + 0.0220) * 2000) / 2000
        
        # LÓGICA DE SINAL BASEADO NO FUTURO VS EQUILÍBRIO
        ...
        # LÓGICA MACRO (SÓ PARA AS SETAS DO RODAPÉ)
        justo_base = round((spot + 0.0310) * 2000) / 2000
        if spot < justo_base - 0.0015: msg_macro, clr_macro, arr = "● DOLAR BARATO", "#00aa55", "▲ ▲ ▲ ▲ ▲"
        elif spot > justo_base + 0.0015: msg_macro, clr_macro, arr = "● DOLAR CARO", "#aa3333", "▼ ▼ ▼ ▼ ▼"
        else: msg_macro, clr_macro, arr = "● DOLAR NEUTRO", "#aaaa00", "◄ ◄ ◄ ► ► ►"
        ...
        with ui_area.container():
            if st.session_state.user_type == "ADM":
                with st.expander("PAINEL ADM"):
                    with st.form("adm_panel"):
                        ...
                        v_global["fraldao"] = c3.number_input("FRALDÃO (PTS)", value=v_global.get("fraldao", 15.0), format="%.1f")
            ...
            st.markdown(f'<div class="s-container" style="border-bottom: 2px solid {clr_macro}77"><div class="s-text" style="color:{clr_macro}">{msg_macro}</div></div>', unsafe_allow_html=True)
            
            st.markdown(f'<div class="d-row"><div class="d-label">PARIDADE GLOBAL</div><div class="d-value" style="color:#cc9900">{(v_global["ajuste"]*(1+(spr/100))):.4f}</div></div>', unsafe_allow_html=True)
            ...
            st.markdown(f'<div class="d-row" style="border-bottom:none; padding: 5px 15px;"><div class="d-label" style="color:#333">DÓLAR FUTURO</div><div class="v-futuro">{dol_futuro:.4f}</div></div>', unsafe_allow_html=True)

            # REGIÃO DE CORREÇÃO (VERTICAL EM DUAS COLUNAS)
            st.markdown(f"""
            ...
            st.markdown(f'<div class="f-bar"><div class="f-line"></div><div class="f-arrows" style="color:{clr_macro}">{arr}</div><div class="f-line"></div><div class="tk-wrap"><div class="tk-move">{btk} {btk} {btk}</div></div></div>', unsafe_allow_html=True)
