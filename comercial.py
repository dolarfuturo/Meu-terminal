...
# Conexão com a nuvem (Google Sheets) - TTL=0 força atualização real
conn = st.connection("gsheets", type=GSheetsConnection)

def carregar_valores_mestre():
    try:
        # Lendo sem cache para garantir que mude na hora
...
if st.session_state.get('show_admin'):
    with st.expander("CONFIGURAÇÕES", expanded=True):
        ...
        if pwd == "1234":
            v_aj, v_ref = carregar_valores_mestre()
            n_aj = st.number_input("NOVO AJUSTE", value=v_aj, format="%.4f")
            n_ref = st.number_input("NOVO REFERENCIAL", value=v_ref, format="%.4f")
...
# LOOP DE ATUALIZAÇÃO INFINITA
while True:
    ajuste_m, ref_m = carregar_valores_mestre()
    m, spread = get_market_data()
    
    if m:
        spot = m["BRL=X"]["p"]
        paridade = ajuste_m * (1 + (spread/100))
