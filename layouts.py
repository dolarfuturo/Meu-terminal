...
def calcular_k97_total(spreed_do_dia, spot_data, ewz_data):
    try:
        ...
        if st.session_state.k97_abertura_base is None:
            st.session_state.k97_abertura_base = spot_data['op'] if spot_data['op'] > 100 else spot_data['op'] * 1000
            st.session_state.k97_delta_acumulado = 0.0; st.session_state.k97_base_forca_ciclo = 0.0
        if dentro_horario_b3 and st.session_state.k97_proximo_timestamp_8m is None:
            st.session_state.k97_abertura_base = preco_spot_atual; st.session_state.k97_proximo_timestamp_8m = agora_timestamp + timedelta(minutes=8)
            st.session_state.k97_delta_acumulado = 0.0; st.session_state.k97_base_forca_ciclo = 0.0; is_reset_moment = True
        if dentro_horario_b3 and st.session_state.k97_proximo_timestamp_8m is not None:
            if agora_timestamp >= st.session_state.k97_proximo_timestamp_8m:
                ...
                st.session_state.k97_delta_acumulado = (deslocamento_8m / 4) / 10
                st.session_state.k97_base_forca_ciclo = st.session_state.k97_delta_acumulado
                st.session_state.k97_abertura_base = preco_spot_atual; st.session_state.k97_proximo_timestamp_8m = agora_timestamp + timedelta(minutes=8); is_reset_moment = True
        if not dentro_horario_b3: st.session_state.k97_proximo_timestamp_8m = None
        st.session_state.k97_delta_acumulado = st.session_state.k97_base_forca_ciclo + ((preco_spot_atual - st.session_state.k97_abertura_base) / 1000)
        
        # Persistência
        with open("delta_k97.txt", "w") as f: f.write(str(st.session_state.k97_delta_acumulado))
        cor_delta = "#00ff88" if st.session_state.k97_delta_acumulado >= 0 else "#ff4d4d"
...
with st.sidebar:
    ...
    if st.button("SALVAR CONFIGURAÇÕES"): st.session_state.div_spreed_mem = i_div; salvar_eixos(i_div); st.success("Salvo!"); time.sleep(0.5); st.rerun()

div_s = st.session_state.div_spreed_mem
placeholder = st.empty()

# =============================================================================
# # BLOCO 6: INTERFACE DO TERMINAL E ITERAÇÃO DE MERCADO (LOOP INTEGRAL FIXO 4S)
# =============================================================================
while True:
    ...
    with placeholder.container():
        ...
        if res:
            ...
            with c1:
                ...
                html_barra = '''<div class="bar-wrapper-full"><div class="force-scale"><span>100%</span><span>50%</span><span>0%</span><span>50%</span><span>100%</span></div><div class="force-container-dual"><div class="center-line"></div><div class="bar-side"><div class="fill-green" style="width: __PV__%;"></div></div><div class="bar-side"><div class="fill-red" style="width: __PR__%;"></div></div></div><div style="display: flex; justify-content: space-between; align-items: center; font-size: 10px; font-family: monospace; color: #AAA; margin-top: 2px; padding: 0 2px;"><span>LOW: __LOW__</span><span style="color:__COR_SETA__; font-size: 14px; font-weight: bold;">__SETA__</span><span>HIGH: __HIGH__</span></div><div class="sinal-indicator __BLINK__" style="color:__SETA_COR__;">__SETA_TXT__</div></div>'''
                html_barra = html_barra.replace("__PV__", str(res["p_v"])).replace("__PR__", str(res["p_r"])).replace("__LOW__", f"{res['alvo_low']:.2f}").replace("__HIGH__", f"{res['alvo_high']:.2f}").replace("__COR_SETA__", cor_seta_spread).replace("__SETA__", seta_spread).replace("__BLINK__", "blink" if res["piscando"] else "").replace("__SETA_COR__", res["seta_cor"]).replace("__SETA_TXT__", res["seta"])
            ...
            with c2:
                ...
                st.markdown(f'''<div class="calc-panel" style="margin-top: 4px;"><div class="calc-row"><span style="color:#AAA;">PREÇO BASE (8M)</span> <span style="color:#ffffff; font-weight: bold;">{res['preco_base_atual']:.4f}</span></div><div class="calc-row" style="border-bottom: none; margin-top: 2px;"><span style="color:#ffffff;">𝚫 SPOT (FORÇA)</span> <span style="color:{res['cor_delta']}; font-size: 14px; font-weight: bold;">{res['delta_spot_forca']:+.3f}</span></div></div>''', unsafe_allow_html=True)
