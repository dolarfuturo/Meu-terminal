...
<style>
    ...
    .clock-box { text-align: center; border: 1.5px solid #ffffff; padding: 4px 10px; border-radius: 4px; background: #0a141a; min-width: 95px; color: #fff; font-family: 'monospace'; }
...
def calcular_k97_total(eixo_ewz, p_ewz_atual, max_ewz, min_ewz, eixo_dol):
    try:
        ...
        return {
            "vivo": dolar_vivo, "max": alvo_max, "min": alvo_min,
...
st.markdown(f'<div class="header-bair"><div><span class="bair-text">BAIR</span> <span style="color:white; font-size:40px;">-</span> <span class="terminal-text">TERMINAL DOLAR</span></div><div class="clock-box">{br_t}</div></div>', unsafe_allow_html=True)
...
if ewz_live:
    ...
    with c_main:
        html_table = '<div class="main-grid"><table class="terminal-table"><thead><tr><th>Ativo</th><th style="color: #d4a017;">Price</th><th>Ref/Close</th><th>Max</th><th>Min</th><th>Var</th></tr></thead><tbody>'
        
        ativos = {"DOLFUT": "DOL=F", "SPOT": "USDBRL=X", "DXY": "DX-Y.NYB", "EWZ": "EWZ", "XAU/USD": "GC=F", "BRENT": "BZ=F"}
        ticker_items = []
        ...
        for lbl, sym in ativos.items():
            ...
            html_table += f"<tr><td class='asset-name'>{lbl}</td><td class='price-col'>{val:{f}}</td><td>{ref:{f}}</td><td>{d['mx']:{f}}</td><td>{d['mn']:{f}}</td><td style='color:{color}; font-weight:bold;'>{var:+.2f}%</td></tr>"
