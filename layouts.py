...
<style>
    ...
    /* BARRA COMPACTADA VERTICALMENTE */
    ...
    .force-scale { display: flex; justify-content: space-between; font-size: 7px; font-family: monospace; color: #AAA; margin-bottom: 1px; padding: 0 5px; }
    .force-container-dual { background: #111; height: 8px; width: 100%; border-radius: 1px; position: relative; overflow: hidden; display: flex; border: 1px solid #444; }
    ...
    .sinal-indicator { font-size: 10px; font-weight: 900; margin-top: 2px; line-height: 1; }
    ...
    with placeholder.container():
        ...
        if res:
            ...
            with c1:
                ...
                st.markdown(f'''<div class="bar-wrapper-full"><div class="force-scale"><span>100%</span><span>50%</span><span>0%</span><span>50%</span><span>100%</span></div><div class="force-container-dual"><div class="center-line"></div><div class="bar-side"><div class="fill-green" style="width: {res["p_v"]}%;"></div></div><div class="bar-side"><div class="fill-red" style="width: {res["p_r"]}%;"></div></div></div><div style="display: flex; justify-content: space-between; font-size: 9px; font-family: monospace; color: #AAA; margin-top: 1px; padding: 0 2px;"><span>LOW: {res['alvo_low']:.2f}</span><span>HIGH: {res['alvo_high']:.2f}</span></div><div class="sinal-indicator {"blink" if res["piscando"] else ""}" style="color:{res["seta_cor"]};">{res["seta"]}</div></div>''', unsafe_allow_html=True)
