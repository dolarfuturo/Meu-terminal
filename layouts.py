...
spot = fetch("USDBRL=X")
ewz = fetch("EWZ")
dolfut_data = fetch("BRL=F")

# LÓGICA DAS SETAS (CÁLCULOS SOLICITADOS)
spreed = (spot['mx'] - spot['mn']) / 8
max_fut_val = a_dol + spot['mx'] + spreed
min_fut_val = a_dol - spot['mn'] + spreed
media_dol_calc = (spot['mx'] + spot['mn']) / 2
...
with col1:
    ...
    for lbl, sym in assets.items():
        d = spot if lbl == "DOLSPOT" else (ewz if lbl == "EWZ" else (dolfut_data if lbl == "DOLFUT" else fetch(sym)))
...
with col2:
    ...
    <div class="calc-panel">
        <div class="calc-row" style="color:#ff4d4d; border-bottom: 2px solid #fff;"><span>MÁXIMA</span> <span>{max_fut_val:.2f}</span></div>
        <div style="text-align:center; padding: 45px 0; color: #00f2ff; font-size: 26px; font-weight: bold;">AXIS: {a_dol:.2f}</div>
        <div class="calc-row" style="color:#00ff88; border-top: 2px solid #fff; border-bottom:none;"><span>MÍNIMA</span> <span>{min_fut_val:.2f}</span></div>
    </div>
    """, unsafe_allow_html=True)
    ...
    st.markdown(f"""
    <div class="calc-panel">
        <div class="calc-row"><span>DOLFUT</span> <span style="color:#00f2ff;">{dolfut_data['at']:.2f}</span></div>
        <div class="calc-row"><span style="color:#ffff00;">MÉDIA DOL</span> <span style="color:#00f2ff;">{media_dol_calc:.2f}</span></div>
