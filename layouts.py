...
# --- CSS: CORES DE LINHA COM DESTAQUE + TICKER FULL WIDTH ---
st.markdown("""
<style>
    .stApp { background-color: #050a0e !important; }
    .main-grid { border: 2.5px solid #1c3d4d; border-radius: 8px; overflow: hidden; font-family: 'monospace'; background-color: #0d1b22; }
    ...
    /* Cores das Linhas com Destaque */
    .terminal-table tr:nth-child(1) { background-color: rgba(0, 242, 255, 0.05); } /* Sintético */
    .terminal-table tr:hover { background-color: rgba(212, 160, 23, 0.1); }
    ...
    .calc-panel { border: 2.5px solid #1c3d4d; border-radius: 8px; padding: 10px; background: #0a141a; font-family: monospace; margin-bottom: 10px; }
    ...
    /* Ticker FULL WIDTH */
    .ticker-wrapper { width: 100vw; position: relative; left: 50%; right: 50%; margin-left: -50vw; margin-right: -50vw; background: #000; border-top: 2px solid #1c3d4d; border-bottom: 2px solid #1c3d4d; padding: 8px 0; overflow: hidden; white-space: nowrap; margin-top: 20px; }
    ...
    .monitor-bar { background: #0a141a; border: 2px solid #1c3d4d; padding: 8px; text-align: center; color: #00f2ff; font-weight: bold; font-family: monospace; border-radius: 4px; }
</style>
...
if ewz_live:
    ...
    with c_main:
        ...
        # Configuração de Ativos com GOLD 3 casas
        ativos_config = {
            ...
            "GOLD": {"sym": "GC=F", "fmt": ".3f"}, # Ajustado para 3 casas
        ...
        for label, cfg in ativos_config.items():
            ...
            if d:
                ...
                html_table += f"<tr><td style='color:#fff; text-align:left; font-weight:bold; padding-left:15px;'>{label}</td><td style='color:#d4a017;'>{d['at']:{cfg['fmt']}}</td><td>{d['cl']:{cfg['fmt']}}</td><td>{d['cl']:{cfg['fmt']}}</td><td>{d['mx']:{cfg['fmt']}}</td><td>{d['mn']:{cfg['fmt']}}</td><td style='color:{c}; font-weight:bold;'>{v:+.2f}%</td></tr>"
    ...
    # Ticker fora das colunas para pegar a largura total
