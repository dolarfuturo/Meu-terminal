...
def fetch(s):
    try:
        ...
        if d.empty: return {"at": 0.0, "cl": 0.1, "mx": 0.0, "mn": 0.0, "op": 0.0}
        m = 1000 if s == "USDBRL=X" else 1
        ref_cl = t.info.get('previousClose', d['Open'].iloc[0]) * m
        return {"at": d['Close'].iloc[-1] * m, "cl": ref_cl, "op": d['Open'].iloc[0] * m, "mx": d['High'].max() * m, "mn": d['Low'].min() * m}
...
while True:
    ...
    # Cálculo Justo
    v_v = (((spot_live['at']/spot_live['cl'])-1)*0.6 - ((ewz_live['at']/ewz_live['cl'])-1)*0.4) * 100
    dfut_calc = st.session_state.a_dol * (1 + (v_v/100))
    
    with placeholder.container():
        st.markdown(f"""<div class="header-bair"><div class="title-box"><span class="bair-text">BAIR</span><span class="terminal-text">-TERMINAL</span></div><div class="clock-box"><span class="clock-label">BRASÍLIA</span><span class="clock-time">{datetime.now(tz_sp).strftime('%H:%M:%S')}</span></div></div>""", unsafe_allow_html=True)
        
        c1, c2 = st.columns([3, 1])
        with c1:
            html = f"""<div class="main-grid"><table class="terminal-table"><thead><tr><th>ATIVO</th><th>PRICE</th><th>MAX</th><th>MIN</th><th>VAR</th></tr></thead><tbody>
            <tr><td class='asset-name'>DOLFUT</td><td class='price-col'>{(dfut_calc/1000):.4f}</td><td>{(spot_live['mx']/1000):.4f}</td><td>{(spot_live['mn']/1000):.4f}</td><td style='color:#00ff00;'>{v_v:+.2f}%</td></tr>
            </tbody></table></div>"""
