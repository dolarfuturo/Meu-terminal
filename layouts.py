...
def calcular_eixo_automatico():
    try:
        ...
        hoje = agora.date()
        idx = -2 if (df.index[-1].date() == hoje and agora.hour < 18) else -1
...
def calcular_k97_total(eixo_ewz, p_ewz_atual, max_ewz, min_ewz, eixo_dol):
    try:
        ...
        return {
            ...
            "v_atual": var_atual, "ewz_med": ewz_med,
            "max": eixo_dol * (1 + (v_pos / 100)), "min": eixo_dol * (1 + (v_neg / 100)),
            "p75_up": (eixo_dol + (eixo_dol * (1 + (v_pos / 100)) - eixo_dol)*0.75), 
            "p50_up": (eixo_dol + eixo_dol * (1 + (v_pos / 100))) / 2, 
            "p25_up": (eixo_dol + (eixo_dol * (1 + (v_pos / 100)) - eixo_dol)*0.25),
            "p75_down": (eixo_dol + (eixo_dol * (1 + (v_neg / 100)) - eixo_dol)*0.75), 
            "p50_down": (eixo_dol + eixo_dol * (1 + (v_neg / 100))) / 2, 
            "p25_down": (eixo_dol + (eixo_dol * (1 + (v_neg / 100)) - eixo_dol)*0.25)
        }
    except: return None
...
if ewz_live:
    ...
    with c_main:
        ...
        html_table = """<div class="main-grid"><table class="terminal-table"><thead><tr><th>Ativo</th><th>Price</th><th>Close/Axis</th><th>Open</th><th>Max</th><th>Min</th><th>Var</th></tr></thead><tbody>"""
    ...
    with c_side:
        ...
        st.markdown(f"""<div class="calc-panel"><div class="calc-row" style="padding: 10px 8px;"><span style="color:#ffffff;">DOLFUT</span> <span style="color:#00f2ff; font-size: 16px; font-weight: 950;">{res['vivo']:.2f}</span></div><div class="calc-row"><span style="color:#ffff00;">MÉDIA DOL</span> <span style="color:#00f2ff; font-size: 16px;">{res['medio']:.2f}</span></div><div class="calc-row" style="border-bottom: none;"><span style="color:#d4a017;">P. JUSTO</span> <span style="color:#ffffff; font-size: 16px; font-weight: bold;">{res['fraja']:.2f}</span></div><div class="ewz-mini-container"><span class="ewz-mini-val" style="color:#00ff88;">{ewz_live['mx']:.2f}</span><span class="ewz-mini-val" style="color:#00f2ff;">{a_ewz:.2f}</span><span class="ewz-mini-val" style="color:#ff4d4d;">{ewz_live['mn']:.2f}</span></div></div>""", unsafe_allow_html=True)
