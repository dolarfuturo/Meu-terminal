...
# --- CSS ORIGINAL (COM AJUSTE NO LOGO) ---
st.markdown("""
<style>
    ...
    .main-title { margin: 0px; line-height: 1.2; font-size: 22px; font-family: monospace; padding-bottom: 5px; } /* TAMANHO REDUZIDO */
    ...
    .sinal-indicator { font-size: 11px; font-weight: 900; margin-top: 4px; }
    .blink { animation: blinker 1.5s linear infinite; }
    @keyframes blinker { 50% { opacity: 0.1; } }
    .ticker-wrapper { width: 100%; background: #000; border-top: 1.5px solid #ffffff; border-bottom: 1.5px solid #ffffff; padding: 4px 0; overflow: hidden; white-space: nowrap; margin-top: 10px; }
    .ticker-text { display: inline-block; padding-left: 100%; animation: marquee 30s linear infinite; font-family: 'monospace'; font-size: 12px; font-weight: bold; color: #fff; }
...
while True:
    ...
    spot_l, ewz_l = fetch("USDBRL=X"), fetch("EWZ")
    j_br, j_us = fetch("^BVSP"), fetch("^TNX")
    
    now = datetime.now()
    with placeholder.container():
        st.markdown(f'''<div class="header-container"><h1 class="main-title"><span class="bair-blue">BAIR</span><span class="terminal-gold"> - TERMINAL DOLLAR</span></h1><div class="clock-row"><span>🇧🇷 BR: <span class="br-green">{now.astimezone(tz_sp).strftime("%H:%M:%S")}</span></span><span>🇺🇸 NY: <span class="white-time">{now.astimezone(tz_ny).strftime("%H:%M:%S")}</span></span><span>🇬🇧 LDN: <span class="white-time">{now.astimezone(tz_ld).strftime("%H:%M:%S")}</span></span><span>🌐 UTC: <span class="utc-gold">{now.astimezone(tz_utc).strftime("%H:%M:%S")}</span></span></div><div class="date-container">📅 {now.astimezone(tz_sp).strftime("%d/%m/%Y")}</div></div>''', unsafe_allow_html=True)

        res = calcular_k97_total(div_s, ewz_l['at'] if ewz_l else 0, a_dol, spot_l)
        if res:
            var_d_axis = ((res['vivo'] / a_dol) - 1) * 100
            ...
            with c1:
                ...
                if j_br and j_us:
                    v_j = ((j_br['at']/j_br['cl'])-1)*100 - ((j_us['at']/j_us['cl'])-1)*100
                    p_j = j_br['at']-j_us['at']; l_j = st.session_state.last_p.get("JUROS", p_j); cl_j = "f-up" if p_j > l_j else "f-dn" if p_j < l_j else ""; st.session_state.last_p["JUROS"] = p_j
                    html += f"<tr><td class='asset-name'>JUROS</td><td class='price-col {cl_j}'>{p_j:.2f}</td><td>{j_br['cl']:.2f}</td><td>{j_us['cl']:.2f}</td><td>{j_br['mx']:.2f}</td><td>{j_br['mn']:.2f}</td><td style='color:{("#00ff00" if v_j >= 0 else "#ff4d4d")}; font-weight:bold;'>{v_j:+.2f}%</td></tr>"
                st.markdown(html + "</tbody></table></div>", unsafe_allow_html=True)
                st.markdown(f'''<div class="bar-wrapper-full"><div class="force-container-dual"><div class="center-line"></div><div class="bar-side"><div class="fill-green" style="width: {res["p_v"]}%;"></div></div><div class="bar-side"><div class="fill-red" style="width: {res["p_r"]}%;"></div></div></div><div style="display: flex; justify-content: space-between; font-size: 10px; font-family: monospace; color: #AAA; margin-top: 2px;"><span>LOW: {res['alvo_low']:.2f}</span><span>HIGH: {res['alvo_high']:.2f}</span></div><div class="sinal-indicator {"blink" if res["piscando"] else ""}" style="color:{res["seta_cor"]};">{res["seta"]}</div></div>''', unsafe_allow_html=True)

            with c2:
                st.markdown('<div class="section-title">CÁLCULOS</div>', unsafe_allow_html=True)
                st.markdown(f'''<div class="calc-panel">
                    <div class="calc-row txt-red"><span>MAX FUT 5</span> <span>{res['max_fut_5']:.2f}</span></div>
                    <div class="calc-row txt-yellow"><span>MAX FUT 4</span> <span>{res['max_fut_4']:.2f}</span></div>
                    <div class="calc-row txt-red"><span>MAX FUT 3</span> <span>{res['max_fut_3']:.2f}</span></div>
                    <div class="calc-row txt-yellow"><span>MAX FUT 2</span> <span>{res['max_fut_2']:.2f}</span></div>
                    <div class="calc-row txt-red"><span>MAX FUT 1</span> <span>{res['max_fut_1']:.2f}</span></div>
                ...
                st.markdown(f'''<div class="calc-panel"><div class="calc-row" style="border-bottom:none;"><span style="color:#ffffff;">DOLB3</span> <span style="color:#00f2ff;">{res['vivo']:.2f}</span></div><div style="text-align:right; font-size:9px; padding-right:6px; color:{("#00ff00" if var_d_axis >= 0 else "#ff4d4d")}; font-weight:bold; margin-bottom:4px;">{var_d_axis:+.2f}%</div><div class="calc-row"><span style="color:#ffff00;">MÉDIA DOLAR</span> <span style="color:#00f2ff;">{res['medio']:.2f}</span></div><div class="calc-row"><span style="color:#d4a017;">PREÇO JUSTO</span> <span style="color:#ffffff;">{res['fraja']:.2f}</span></div><div class="calc-row" style="border-bottom: none;"><span style="color:#ff4d4d;">SPREED</span> <span style="color:#00f2ff;">{res['spreed']:.2f}</span></div></div>''', unsafe_allow_html=True)

        st.markdown(f'''<div class="ticker-wrapper"><div class="ticker-text">BAIR TERMINAL USD | DOLFUT: {res["vivo"]:.2f} | VAR: {res["v_v"]:+.2f}% | SPREED: {res["spreed"]:.2f} | REGIÃO: {res["seta"]} | AXIS ATUAL: {a_dol:.2f}</div></div>''', unsafe_allow_html=True)
    time.sleep(5)
