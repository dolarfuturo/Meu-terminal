...
def calcular_k97_total(eixo_ewz, p_ewz_atual, max_ewz, min_ewz, eixo_dol, spot_data):
    try:
        ...
        # --- SPREDD E MÉDIA (USANDO O SPOT) ---
        spreedd = (spot_data['mx'] - spot_data['mn']) / 8
        media_dol_spot = (spot_data['mx'] + spot_data['mn']) / 2
        
        # --- MÁXIMAS E MÍNIMAS DO FUTURO ---
        ...
        return {
            "vivo": dolar_vivo, "fraja": dolar_fraja, "medio": media_dol_spot, "ewz_med": ewz_medio_dia,
            "max": alvo_max, "min": alvo_min, "v_v": v_final * 100, "spreedd": spreedd
        }
    except: return None
...
if res:
    ...
    with c_side:
        # PAINEL DA SETA VERMELHA AJUSTADO (SÓ AXIS, MAX E MIN)
        st.markdown(f"""
        <div class="calc-panel">
            <div class="calc-row" style="color:#ff4d4d; font-size: 15px;"><span>MÁXIMA</span> <span>{res['max']:.2f}</span></div>
            <div style="text-align:center; padding: 15px; color: #00f2ff; font-size: 22px; font-weight: 950; border-top:1.5px solid #444; border-bottom:1.5px solid #444; margin: 10px 0;">AXIS: {a_dol:.2f}</div>
            <div class="calc-row" style="color:#00ff88; border-bottom: none; font-size: 15px;"><span>MÍNIMA</span> <span>{res['min']:.2f}</span></div>
        </div>""", unsafe_allow_html=True)
        
        # PAINEL DA SETA VERDE AJUSTADO (MÉDIA DOL = (MAX+MIN)/2 DO SPOT)
        st.markdown(f"""
        <div class="calc-panel">
            ...
            <div class="ewz-mini-container"><span class="ewz-mini-val" style="color:#00ff88;">{ewz_live['mx']:.2f}</span><span class="ewz-mini-val" style="color:#00f2ff;">{res['ewz_med']:.2f}</span><span class="ewz-mini-val" style="color:#ff4d4d;">{ewz_live['mn']:.2f}</span></div>
        </div>""", unsafe_allow_html=True)
