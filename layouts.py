...
# --- CSS: REMOÇÃO DE TOPO E AJUSTE DE ENCAIXE ---
st.markdown("""
<style>
    /* 1. LIMPEZA DO TOPO (BARRA PRETA E BONECOS) */
    ...
    /* 3. BOTÃO ADM (Sempre visível no canto) */
    .admin-btn-container { position: fixed; top: 10px; left: 10px; z-index: 9999; }

    /* 4. GRADE E TABELA (ORIGINAL) */
    ...
    /* 6. COMPRESSÃO DOS BLOCOS DA DIREITA */
    .calc-panel { border: 2.5px solid #ffffff; border-radius: 8px; padding: 6px; background: #0a141a; font-family: monospace; margin-bottom: 5px; }
    .calc-row { display: flex; justify-content: space-between; padding: 3px 8px; border-bottom: 1px solid #444; font-size: 12px; font-weight: bold; align-items: center; }
    
    /* 7. RODAPÉ SUBINDO (COLADO) */
    .ticker-wrapper { width: 100vw; position: relative; left: 50%; right: 50%; margin-left: -50vw; margin-right: -50vw; background: #000; border-top: 2px solid #ffffff; border-bottom: 2px solid #ffffff; padding: 6px 0; overflow: hidden; white-space: nowrap; margin-top: 2px; }
    ...
    .monitor-bar { background: #0a141a; border: 2.2px solid #ffffff; padding: 4px; text-align: center; color: #00f2ff; font-weight: bold; font-family: monospace; border-radius: 4px; margin-bottom: 4px; font-size: 13px; }
...
# --- BOTÃO ADM (Abre a Sidebar) ---
st.markdown('<div class="admin-btn-container">', unsafe_allow_html=True)
if st.button("⚙️"):
    st.info("Use a barra lateral para ajustar os valores.")
st.markdown('</div>', unsafe_allow_html=True)

# --- MOTOR DE DADOS (RESTAURADO) ---
def fetch(s):
    try:
        t = yf.Ticker(s)
...
def calcular_k97_total(eixo_ewz, p_ewz_atual, max_ewz, min_ewz, eixo_dol):
    try:
        ...
        return {
            "vivo": eixo_dol * (1 + (v_at / 100)), "fraja": eixo_dol * (1 + (v_fr / 100)), "medio": eixo_dol * (1 + (var_medio / 100)), 
...
if res:
    ...
    with c_main:
        ...
        for lbl, sym in outros.items():
            d = fetch(sym); f = ".4f" if "USD" in lbl or lbl == "SPOT" else ".2f"
    ...
    with c_side:
        # PROJEÇÕES COM TODAS AS VARIÁVEIS RESTAURADAS
        st.markdown(f"""<div class="calc-panel">
            ...
            <div style="text-align:center; padding: 5px; color: #00f2ff; font-size: 16px; font-weight: bold; border-top:1.5px solid #444; border-bottom:1.5px solid #444; margin: 4px 0;">AXIS: {a_dol:.2f}</div>
        ...
        # SEGUNDO BLOCO COMPRIMIDO
        st.markdown(f"""<div class="calc-panel">
            <div class="calc-row"><span style="color:#fff;">DOLFUT</span> <span style="color:#00f2ff;">{res['vivo']:.2f}</span></div>
