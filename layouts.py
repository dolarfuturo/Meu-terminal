...
# --- MOTOR DE DADOS: EIXO RESTRITO 10:30-17:00 ---
@st.cache_data(ttl=600)
def calcular_eixo_automatico():
    try:
        t = yf.Ticker("EWZ")
        # Puxa histórico minuto a minuto para filtrar o horário regular
        ...
        # Seleciona o dia de referência (ontem ou hoje após as 18h)
        datas = sorted(list(set(df.index.date)))
        target_date = datas[-1]
        if target_date == agora.date() and agora.hour < 18:
            target_date = datas[-2] if len(datas) > 1 else target_date
            
        df_dia = df[df.index.date == target_date]
        # FILTRO DO EIXO: APENAS HORÁRIO REGULAR
        df_regular = df_dia.between_time(dt_time(10, 30), dt_time(17, 0))
        
        if not df_regular.empty:
            mx, mn = df_regular['High'].max(), df_regular['Low'].min()
...
def calcular_k97_total(eixo_ewz, p_ewz_atual, max_ewz, min_ewz, eixo_dol):
    try:
        ...
        var_med = ((eixo_ewz / ewz_med) - 1) * 100 
        dolar_med = eixo_dol * (1 + (var_med / 100)) 
        ...
        return {
            "vivo": dolar_vivo, "fraja": dolar_fraja, "medio": dolar_med, 
...
def fetch(s):
    try:
        # BUSCA REAL-TIME (COM PRÉ E PÓS MERCADO)
...
if ewz_live:
    ...
    with c_main:
        ...
        ticker_items = [f"DOLFUT: {v_var:+.2f}%" if 'v_var' in locals() else "DOLFUT: 0.00%"]
    ...
    with c_side:
        ...
        st.markdown(f"""<div class="calc-panel"><div class="calc-row"><span>DOLFUT</span> <span style="color:#00f2ff;">{res['vivo']:.2f}</span></div><div class="calc-row" style="border-bottom:none;"><span>JUSTO</span> <span style="color:#ffffff;">{res['fraja']:.2f}</span></div><div class="ewz-mini-container"><span class="ewz-mini-val" style="color:#00ff88;">{mx_ref:.2f}</span><span class="ewz-mini-val" style="color:#00f2ff;">{a_ewz:.2f}</span><span class="ewz-mini-val" style="color:#ff4d4d;">{mn_ref:.2f}</span></div></div>""", unsafe_allow_html=True)
