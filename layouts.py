...
<style>
    ...
    .elastic-row { display: flex; justify-content: space-between; padding: 2px 10px; font-family: monospace; font-size: 11px; font-weight: bold; margin-top: 5px; border-top: 1px solid #333; }
</style>
...
def calcular_k97_total(div_spreed, p_ewz_atual, eixo_dol, spot_data):
    try:
        ...
        # 2. DEFINIÇÃO DO ELÁSTICO
        max_original = eixo_dol + (amp * 0.75)
        min_original = eixo_dol - (amp * 0.25)
        dolar_medio = ((max_original + min_original) / 2) - v_spreed
        elastico_calculado = abs(eixo_dol - dolar_medio)
        
        # 3. LÓGICA DO ARQUITETO (TRAVADA POR PARENTESES)
        # X = AXIS - ELASTICO - MIN SPOT
        val_x = float(eixo_dol - elastico_calculado - spot_data['mn'])
        # LOW = (AXIS - ELASTICO) + X
        low_e = float((eixo_dol - elastico_calculado) + val_x)
        
        # Y = AXIS + ELASTICO - MAX SPOT
        val_y = float(eixo_dol + elastico_calculado - spot_data['mx'])
        # HIGH = (AXIS + ELASTICO) - Y
        high_e = float((eixo_dol + elastico_calculado) - val_y)
    ...
    with placeholder.container():
        ...
        if res:
            ...
            with c1:
                ...
                # BARRA COM O LOW E HIGH REVISADOS
                st.markdown(f'''
                <div class="bar-wrapper-full">
                    ...
                    <div class="elastic-row">
                        <span style="color:#00f2ff;">LOW: {res['low_e']:.2f}</span>
                        <span style="color:#00f2ff;">HIGH: {res['high_e']:.2f}</span>
                    </div>
