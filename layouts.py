...
<style>
    ...
    /* GAVETA LATERAL */
    section[data-testid="stSidebar"] { background-color: #0a141a !important; border-right: 2px solid #d4a017 !important; }
    ...
    .asset-name { font-size: 17px; color: #fff; text-align: left !important; font-weight: bold; padding-left: 15px !important; }
    ...
    /* BARRA DE FORÇA */
    .bar-wrapper { background: #0a141a; padding: 15px; border: 2.5px solid #ffffff; border-radius: 8px; text-align: center; }
    .force-container { background: #111; height: 20px; width: 100%; position: relative; display: flex; border: 1px solid #444; overflow: hidden; }
    .fill-green { background: #00ff88; height: 100%; transition: width 0.5s; }
    .fill-red { background: #ff4d4d; height: 100%; transition: width 0.5s; }
    .blink { animation: blinker 1s linear infinite; }
    @keyframes blinker { 50% { opacity: 0; } }
</style>
""", unsafe_allow_html=True)

# --- 4. GAVETA LATERAL (ADM) ---
with st.sidebar:
    st.markdown("<h2 style='color:#d4a017;'>⚙️ AJUSTE EIXO</h2>", unsafe_allow_html=True)
    with st.form("set_form"):
        ...
        if st.form_submit_button("APLICAR NO TERMINAL"):
            st.session_state.a_ewz = new_ewz
            st.session_state.a_dol = new_dol
            st.rerun()

# --- 5. FUNÇÃO DE DADOS BLINDADA ---
def fetch_safe(s):
    try:
        t = yf.Ticker(s)
...
while True:
    ...
    pr = min(100, abs(diff)/15 * 100) if diff > 0 else 0
    pv = min(100, abs(diff)/15 * 100) if diff < 0 else 0
    ...
    with main_space.container():
        ...
        with c1:
            # Tabela (Com proteção de formatação para evitar o erro da imagem)
            ...
            for n, at, cl, mx, mn, vr in ativos:
                # Se for dólar, divide por 1000 para exibição
                div = 1000 if "DOL" in n else 1
                f = ".4f" if "DOL" in n else ".2f"
                color = "#00ff88" if vr >= 0 else "#ff4d4d"
                
                html += f"""<tr>
                    <td class="asset-name">{n}</td>
                    <td class="price-col">{ (at/div):{f} }</td>
                    <td>{ (cl/div):{f} }</td>
                    <td>{ (mx/div):{f} }</td>
                    <td>{ (mn/div):{f} }</td>
                    <td style="color:{color}; font-weight:bold;">{vr:+.2f}%</td>
                </tr>"""
            st.markdown(html + "</table></div>", unsafe_allow_html=True)

        with c2:
            # Painel Lateral
            st.markdown(f"""<div style="border:2.5px solid #fff; border-radius:8px; padding:15px; background:#0a141a; text-align:center; margin-bottom:10px;">
                <div style="color:#d4a017; font-size:12px;">AXIS ATUAL</div>
                <div style="color:#fff; font-size:28px; font-weight:bold;">{st.session_state.a_dol:.2f}</div>
                <div style="color:#00f2ff; margin-top:10px;">JUSTO: {p_justo:.2f}</div>
            ...
            st.markdown(f"""<div class="bar-wrapper">
                ...
                <div class="blink" style="margin-top:15px; font-weight:bold; color:{('#00ff88' if pv>pr else '#ff4d4d')}">
                    {('▲ COMPRA FORTE' if pv>85 else '▼ VENDA FORTE' if pr>85 else 'AGUARDANDO')}
                </div>
            </div>""", unsafe_allow_html=True)
