...
# --- CSS PARA ALINHAMENTO E VOLTA DOS ELEMENTOS ---
st.markdown("""
<style>
    ...
    .terminal-table td { border: 1px solid #ffffff; padding: 12px; text-align: center; font-size: 14px; }
    .asset-name { font-size: 16px; color: #fff; text-align: left; font-weight: bold; padding-left: 15px; }

    /* Blocos da Direita Esticados para Alinhar */
    .right-col-container { display: flex; flex-direction: column; height: 100%; gap: 12px; }
    .calc-panel { 
        border: 2.5px solid #ffffff; border-radius: 8px; background: #0a141a; 
        font-family: monospace; padding: 10px 5px; flex-grow: 1;
        display: flex; flex-direction: column; justify-content: space-between;
    }
    .calc-row { display: flex; justify-content: space-between; padding: 6px 10px; border-bottom: 1px solid #333; font-size: 13px; font-weight: bold; }
...
with st.sidebar:
    ...
    a_dol = st.number_input("AXIS DOLFUT", value=5274.0, step=1.0)
    st.button("RECARREGAR TELA")

# --- HEADER COM RELÓGIOS ---
tz_sp, tz_ny, tz_ld = pytz.timezone('America/Sao_Paulo'), pytz.timezone('America/New_York'), pytz.timezone('Europe/London')
st.markdown(f"""<div style="display:flex; justify-content:space-between; align-items:center; padding:10px; border-bottom:2.5px solid #fff; margin-bottom:12px;">
...
if res:
    ...
    with c1:
        st.markdown('<div style="background:#0a141a; border:2px solid #fff; padding:5px; text-align:center; color:#00f2ff; font-weight:bold; margin-bottom:8px;">GRADE PRINCIPAL</div>', unsafe_allow_html=True)
    ...
    with c2:
        # Bloco das Projeções (com flex para ocupar espaço)
        st.markdown('<div style="background:#0a141a; border:2px solid #fff; padding:5px; text-align:center; color:#00f2ff; font-weight:bold; margin-bottom:8px;">PROJEÇÕES</div>', unsafe_allow_html=True)
        st.markdown(f"""<div class="right-col-container" style="height: 520px;">
            ...
            <div class="calc-panel" style="max-height: 140px;">
                ...
                <div style="display:flex; justify-content:space-around; padding-top:5px; border-top:1px solid #444;">
