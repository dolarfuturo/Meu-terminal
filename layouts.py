    ...
    if os.path.exists("config_axis.txt"):
        try:
            with open("config_axis.txt", "r") as f:
                dados = f.read().split(",")
                # Tenta carregar os 3 valores, se não houver o terceiro, usa o eixo_dol como padrão
                d1 = float(dados[0])
                d2 = float(dados[1])
...
while True:
    ...
    with placeholder.container():
        ...
        if res:
            # NOVA VARIAÇÃO CALCULADA PELO AXIS FUT
            ...
            with c1:
                ...
                seta_spread, cor_seta_spread = ("▲", "#00ff88") if d_c > spot_live['at'] else ("▼", "#ff4d4d")
