...
# --- CONTROLE AUTOMÁTICO DE LIQUIDAÇÃO / RESET (BINANCE UTC) ---
if 'ultimo_dia_utc' not in st.session_state: 
    st.session_state.ultimo_dia_utc = datetime.now(pytz.utc).strftime("%Y-%m-%d")
...
# --- CÁLCULOS K97 ---
def calcular_k97_total(spreed_do_dia, p_ewz_atual, eixo_dol, spot_data, us10y_data, dxy_data):
    try:
        if not spot_data or p_ewz_atual == 0 or not us10y_data or not dxy_data: return None
        ...
        # Variação internacional ponderada para precificação derivativa
        calc_variacoes_pct = (v_dxy * 0.7) - (v_ewz * 0.3)
        
        # Preço Justo referenciado no Fechamento do Spot
        ...
        # Elástico calibrado (100% no tamanho do Spread T)
        ...
        # Ancoragem do preço calculado do DOLFUT no Axis Dinâmico
        dolfut_atual_calc = axis_dinamico * (1 + calc_variacoes_pct)
        
        # Rastreamento com persistência e travas operacionais de horário
        tz_sp = pytz.timezone('America/Sao_Paulo')
...
while True:
    ...
    # --- SISTEMA DE RESET AUTOMÁTICO NA ABERTURA DA BINANCE (00:00 UTC) ---
    dia_atual_utc = datetime.now(tz_utc).strftime("%Y-%m-%d")
    if dia_atual_utc != st.session_state.ultimo_dia_utc:
        st.session_state.ultimo_dia_utc = dia_atual_utc
        ...
        if os.path.exists("dolfut_history.txt"):
            try: os.remove("dolfut_history.txt")
            except: pass
        st.rerun()

    # Chamadas otimizadas (evita gargalo e requisição duplicada no loop)
    spot_live, ewz_live, us10y_live, dxy_live = fetch("USDBRL=X"), fetch("EWZ"), fetch("^TNX"), fetch("DX-Y.NYB")
    now = datetime.now()
    ...
    with placeholder.container():
        ...
        res = calcular_k97_total(div_s, ewz_live['at'] if ewz_live else 0, a_dol, spot_live, us10y_live, dxy_live)
        if res:
