...
# Configuração para Tablet - Ocultando menus nativos
st.set_page_config(layout="wide", page_title="BAIR - TERMINAL DOLLAR", initial_sidebar_state="collapsed")

# --- CSS: ESTILIZAÇÃO E REMOÇÃO DA BARRA DE CARREGAMENTO (BONEQUINHO) ---
st.markdown("""
<style>
    /* REMOVE O BONEQUINHO E A BARRA DE ATUALIZAÇÃO DO TOPO */
...
def fetch(s):
    try:
        t = yf.Ticker(s)
        # Usando fast_info para evitar que o Streamlit mostre carregamento
        f = t.fast_info
        return {
            "at": f['last_price'], "cl": f['previous_close'], "op": f['open'],
            "mx": f['day_high'], "mn": f['day_low']
        }
...
# --- BARRA LATERAL COM BOTÃO DE ACESSO ---
with st.sidebar:
    st.markdown("### ⚙️ TERMINAL CONTROL")
    with st.expander("🔓 ACESSAR PAINEL ADM"):
...
# --- LOOP INFINITO (ESTILO CRYPTO) ---
while True:
    try:
        tz_sp, tz_ny, tz_ld = pytz.timezone('America/Sao_Paulo'), pytz.timezone('America/New_York'), pytz.timezone('Europe/London')
        
        # BUSCA DADOS PRIMEIRO (Para não deixar a tela vazia)
        ewz_live = fetch("EWZ")
        res = calcular_k97_total(st.session_state.a_ewz, ewz_live['at'], ewz_live['mx'], ewz_live['mn'], st.session_state.a_dol)
        ...
        if res:
            with placeholder.container():
                ...
                with c1:
                    ...
                    html_table = f"""<div class="main-grid"><table class="terminal-table"><thead><tr><th>Ativo</th><th style='color: #d4a017;'>Price</th><th style='color: #d4a017;'>Close</th><th>Open</th><th>Max</th><th>Min</th><th>Var</th></tr></thead><tbody>
                    <tr><td class='asset-name'>DOLFUT</td><td class='price-col'>{(res['vivo']/1000):.4f}</td><td>{(st.session_state.a_dol/1000):.4f}</td><td>{(st.session_state.a_dol/1000):.4f}</td><td>{(res['max']/1000):.4f}</td><td>{(res['min']/1000):.4f}</td><td style='color:{("#00ff00" if v_v >= 0 else "#ff4d4d")}; font-weight:bold;'>{v_v:+.2f}%</td></tr>"""
                    ...
                    for lbl, sym in outros.items():
                        ...
                        html_table += f"<tr><td class='asset-name'>{lbl}</td><td class='price-col'>{d['at']:{f}}</td><td>{d['cl']:{f}}</td><td>{d['op']:{f}}</td><td>{d['mx']:{f}}</td><td>{d['mn']:{f}}</td><td style='color:{("#00ff00" if var >=0 else "#ff4d4d")}; font-weight:bold;'>{var:+.2f}%</td></tr>"
