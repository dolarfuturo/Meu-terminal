...
def calcular_k97_total(eixo_ewz, p_ewz_atual, max_ewz, min_ewz, eixo_dol, spot_data):
    try:
        ...
        v_ewz_var = ((p_ewz_atual / fetch("EWZ")['cl']) - 1) if fetch("EWZ")['cl'] > 0 else 0
        v_final = (v_spot * 0.6) - (v_ewz_var * 0.4)
        
        # AJUSTE SETA VERMELHA: SPREED E NOVAS MÁXIMAS/MÍNIMAS
        spreed = (spot_data['mx'] - spot_data['mn']) / 8
        alvo_max = eixo_dol + spot_data['mx'] + spreed
        alvo_min = eixo_dol - spot_data['mn'] + spreed
        
        # AJUSTE SETA VERDE: MÉDIA DOL
        media_dol_ajustada = (spot_data['mx'] + spot_data['mn']) / 2
        
        # Mantendo P. JUSTO e EWZ Médio original do código
        ...
        return {
            ...
            "fraja": dolar_fraja, 
            "medio": media_dol_ajustada, 
            "ewz_med": ewz_medio_dia,
            "max": alvo_max, 
            "min": alvo_min, 
            "v_v": v_final * 100,
...
if res:
    ...
    with c_side:
        # BLOCO SUPERIOR (AJUSTADO PELO SPREED)
        ...
        # BLOCO INFERIOR (MÉDIA DOL AJUSTADA)
