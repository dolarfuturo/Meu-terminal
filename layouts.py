...
def calcular_k97_total(eixo_ewz, p_ewz_atual, max_ewz, min_ewz, eixo_dol):
    try:
        # Base de cálculo conforme sua lógica de mercado
...
if ewz_live:
    ...
    with c_main:
        ...
        # 1. DOLFUT (Único baseado no eixo)
        ...
        # 2. OUTROS ATIVOS (Todos Normais)
        outros = {"SPOT": "USDBRL=X", "DXY": "DX-Y.NYB", "EWZ": "EWZ", "XAU/USD": "GC=F", "BRENT": "BZ=F"}
        for lbl, sym in outros.items():
            d = fetch(sym)
            f = ".4f" if "USD" in lbl or lbl == "SPOT" else ".2f"
            
            # Variação Normal de Mercado
    ...
    with c_side:
        ...
        # PAINEL SINTÉTICO (DOLFUT, MÉDIA, JUSTO e EWZ MINI)
