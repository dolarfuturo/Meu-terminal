...
# 2. SEUS MOTORES DE DADOS (SEM ALTERAÇÃO)
def fetch(s):
    try:
        t = yf.Ticker(s)
...
def calcular_k97_total(p_ewz_atual, max_ewz, min_ewz, eixo_dol, spot_data):
    try:
        ...
        p_v, p_r = 0, 0
        if diff < 0: p_v = min(100, (abs(diff)/10)*100)
        else: p_r = min(100, (abs(diff)/10)*100)

        seta_txt, seta_cor = "", "#000000"
        if p_v >= 90: seta_txt, seta_cor = "▲ COMPRA", "#00ff88"
        elif p_r >= 90: seta_txt, seta_cor = "▼ VENDA", "#ff4d4d"
...
# 3. BOTAO SET (GATILHO NEXUS NO TOPO)
with st.expander("▶ SET ADM (EIXOS)"):
...
# 4. PALCO NEXUS (PARA NÃO PISCAR)
nexus_stage = st.empty()

while True:
    ewz_live = fetch("EWZ")
    spot_live = fetch("USDBRL=X")
    res = calcular_k97_total(ewz_live['at'], ewz_live['mx'], ewz_live['mn'], st.session_state.a_dol, spot_live)
    ...
    with nexus_stage.container():
        ...
        if res:
            ...
            with c_main:
                ...
                outros = {"DOLSPOT": "USDBRL=X", "DXY": "DX-Y.NYB", "EWZ": "EWZ", "XAU/USD": "GC=F", "BRENT": "BZ=F"}
                for lbl, sym in outros.items():
                    d = spot_live if lbl == "DOLSPOT" else (ewz_live if lbl == "EWZ" else fetch(sym))
                    f = ".4f" if lbl == "DOLSPOT" else ".2f"
                    var = ((d['at'] / d['cl']) - 1) * 100 if d['cl'] > 0 else 0
                    html_table += f"<tr><td class='asset-name'>{lbl}</td><td class='price-col'>{(d['at']/1000 if lbl=='DOLSPOT' else d['at']):{f}}</td><td>{(d['cl']/1000 if lbl=='DOLSPOT' else d['cl']):{f}}</td><td>{(d['op']/1000 if lbl=='DOLSPOT' else d['op']):{f}}</td><td>{(d['mx']/1000 if lbl=='DOLSPOT' else d['mx']):{f}}</td><td>{(d['mn']/1000 if lbl=='DOLSPOT' else d['mn']):{f}}</td><td style='color:{("#00ff00" if var>=0 else "#ff4d4d")}; font-weight:bold;'>{var:+.2f}%</td></tr>"
                    ticker_list = ticker_items.append(f"{lbl}: <span style='color:{("#00ff00" if var>=0 else "#ff4d4d")};'>{var:+.2f}%</span>")
                st.markdown(html_table + "</tbody></table></div>", unsafe_allow_html=True)

            with c_side:
                # Seus Painéis da Direita
                ...
                st.markdown(f"""<div class="calc-panel">
                    <div style="padding: 10px 8px; border-bottom: 1px solid #444;">
                        <div style="display: flex; justify-content: space-between;">
                            <span style="color:#ffffff;">DOLFUT</span> <span style="color:#00f2ff; font-weight: bold;">{dolfut_com_spread:.2f}</span>
                        </div>
                    </div>
                    <div class="calc-row"><span>JUSTO</span> <span>{res['fraja']:.2f}</span></div>
                    <div class="calc-row" style="border:none;"><span>SPREED</span> <span style="color:#00f2ff;">{res['spreed']:.2f}</span></div>
                    <div class="ewz-mini-container"><span class="ewz-mini-val" style="color:#00ff88;">{ewz_live['mx']:.2f}</span><span class="ewz-mini-val" style="color:#ff4d4d;">{ewz_live['mn']:.2f}</span></div>
                </div>""", unsafe_allow_html=True)
