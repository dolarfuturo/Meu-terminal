...
# --- CSS: TRAVA DE 11CM E DADOS COMPLETOS ---
st.markdown("""
<style>
    ...
    .main-grid { border: 2px solid #ffffff; border-radius: 6px; overflow: hidden; background-color: #0d1b22; }
    .terminal-table { width: 100%; border-collapse: collapse; color: #e0e0e0; font-family: monospace; }
    .terminal-table th { background-color: #0a141a; color: #d4a017; border: 1px solid #ffffff; padding: 5px; font-size: 10px; }
    ...
    .sinal-indicator { font-size: 14px; font-weight: 950; margin-top: 3px; }
    .blink { animation: blinker 1s linear infinite; }
    @keyframes blinker { 50% { opacity: 0.1; } }

    .ticker-wrapper { position: fixed; bottom: 0; left: 0; width: 100%; background: #000; border-top: 2px solid #ffffff; padding: 5px 0; overflow: hidden; white-space: nowrap; }
    ...
    with placeholder.container():
        ...
        if res:
            ...
            with c_main:
                ...
                html_table += f"<tr><td class='asset-name'>DOLFUT</td><td style='{bg_dol}'>{(dolfut_calc/1000):.4f}</td><td>{(a_dol/1000):.4f}</td><td>{(a_dol/1000):.4f}</td><td>{(res['max_fut']/1000):.4f}</td><td>{(res['min_fut']/1000):.4f}</td><td style='color:{("#0ff" if v_v>=0 else "#f44")};'>{v_v:+.2f}%</td></tr>"
                ...
                for lbl, sym in outros.items():
                    ...
                    html_table += f"<tr><td class='asset-name'>{lbl}</td><td>{d['at']/1000 if lbl=='DOLSPOT' else d['at']:.2f}</td><td>{d['cl']/1000 if lbl=='DOLSPOT' else d['cl']:.2f}</td><td>{d['op']/1000 if lbl=='DOLSPOT' else d['op']:.2f}</td><td>{d['mx']/1000 if lbl=='DOLSPOT' else d['mx']:.2f}</td><td>{d['mn']/1000 if lbl=='DOLSPOT' else d['mn']:.2f}</td><td>{var:+.2f}%</td></tr>"
                    ticker_items.append(f"{lbl}: {var:+.2f}%")
                st.markdown(html_table + "</tbody></table></div>", unsafe_allow_html=True)
            
            with c_side:
                st.markdown('<div class="section-title">PROJEÇÕES</div>', unsafe_allow_html=True)
                # DADOS RESTAURADOS AQUI:
                st.markdown(f"""<div class="calc-panel">
                    ...
                    <div style="text-align:center; color:#0ff; font-weight:bold; padding:5px; border-top:1px solid #444; border-bottom:1px solid #444;">AXIS: {a_dol:.2f}</div>
                ...
                # SEGUNDO BLOCO COM MÉDIA E JUSTO
                st.markdown(f"""<div class="calc-panel">
                    <div class="calc-row"><span>MÉDIA DOL</span><span>{res['medio']:.2f}</span></div>
                    <div class="calc-row"><span>P. JUSTO</span><span>{res['fraja']:.2f}</span></div>
                    <div class="calc-row" style="border-bottom:none; color:#ff4d4d;"><span>SPREAD</span><span>{res['spreed']:.2f}</span></div>
                </div>""", unsafe_allow_html=True)
                
                # BARRA COM ESCALA 100/80/50/30
                st.markdown(f"""<div class="bar-wrapper-dual">
                    <div class="force-scale">
                        <div class="scale-pos"><span>100%</span><span>80%</span><span>50%</span><span>30%</span></div>
                        <div class="scale-neg"><span>30%</span><span>50%</span><span>80%</span><span>100%</span></div>
                    </div>
