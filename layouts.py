...
# --- CSS: ESTILIZAÇÃO AJUSTADA (LAYOUT SOLTO PARA AJUSTE) ---
st.markdown("""
<style>
    /* Layout solto para evitar corte no topo - Ajuste o padding-top conforme necessário */
    ...
    .main-title { margin: 0px; line-height: 1.1; font-size: 32px; font-family: monospace; }
    ...
    /* PAINÉIS LATERAIS - DADOS COMPLETOS RECOLOCADOS */
...
while True:
    ...
    with placeholder.container():
        ...
        if res:
            ...
            with c_side:
                st.markdown('<div class="section-title">CÁLCULOS PROJEÇÕES</div>', unsafe_allow_html=True)
                st.markdown(f"""<div class="calc-panel">
                    <div class="calc-row" style="color:#ff4d4d;"><span>MAX FUT</span> <span>{res['max_fut']:.2f}</span></div>
                    <div class="calc-row" style="color:#ffa500;"><span>75% UP</span> <span>{res['p75_up']:.2f}</span></div>
                    <div class="calc-row" style="color:#ffa500;"><span>25% UP</span> <span>{res['p25_up']:.2f}</span></div>
                    ...
                    <div class="calc-row" style="color:#ffa500;"><span>25% DOWN</span> <span>{res['p25_down']:.2f}</span></div>
                    <div class="calc-row" style="color:#ffa500;"><span>75% DOWN</span> <span>{res['p75_down']:.2f}</span></div>
                    <div class="calc-row" style="color:#00ff88; border-bottom: none;"><span>MIN FUT</span> <span>{res['min_fut']:.2f}</span></div>
                </div>""", unsafe_allow_html=True)
                
                st.markdown(f"""<div class="calc-panel">
                    <div class="calc-row"><span style="color:#ffffff;">DOLFUT</span> <span style="color:#00f2ff; font-size: 16px;">{dolfut_com_spread:.2f}</span></div>
