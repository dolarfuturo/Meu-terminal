...
def calcular_k97_total(eixo_ewz, p_ewz_atual, max_ewz, min_ewz, eixo_dol, spot_data):
    try:
        ...
        # --- CÁLCULO MÁXIMA E MÍNIMA (IMAGEM 1000035823) ---
        alvo_max = spot_data['mx'] + v_spreed
        alvo_min = spot_data['mn'] + v_spreed
        
        # --- MÉDIA DOL CONFORME INSTRUÇÃO (MAX SPOT+ MAX SPOT)/2 ---
        dolar_medio = (spot_data['mx'] + spot_data['mx']) / 2
        
        # --- 50% (IMAGEM 1000035823) ---
        ...
        return {
            ...
            "ewz_med": (max_ewz + min_ewz) / 2, "max": alvo_max, "min": alvo_min, 
            "v_v": v_final * 100, "spreed": v_spreed, "p50_up": p50_up, "p50_down": p50_down
        }
    except: return None
...
if res:
    ...
    with c_side:
        # PAINEL LATERAL LIMPO CONFORME IMAGEM 1000035823
        st.markdown(f"""<div class="calc-panel"><div class="calc-row" style="color:#ff4d4d;"><span>MÁXIMA</span> <span>{res['max']:.2f}</span></div><div class="calc-row" style="color:#ffa500;"><span>50%</span> <span>{res['p50_up']:.2f}</span></div><div style="text-align:center; padding: 10px; color: #00f2ff; font-size: 18px; font-weight: bold; border-top:1.5px solid #444; border-bottom:1.5px solid #444; margin: 5px 0;">AXIS: {a_dol:.2f}</div><div class="calc-row" style="color:#ffa500;"><span>50%</span> <span>{res['p50_down']:.2f}</span></div><div class="calc-row" style="color:#00ff88; border-bottom: none;"><span>MÍNIMA</span> <span>{res['min']:.2f}</span></div></div>""", unsafe_allow_html=True)
        # PAINEL INFERIOR
        st.markdown(f"""<div class="calc-panel"><div class="calc-row" style="padding: 10px 8px;"><span style="color:#ffffff;">DOLFUT</span> <span style="color:#00f2ff; font-size: 16px; font-weight: 950;">{res['vivo']:.2f}</span></div><div class="calc-row"><span style="color:#ffff00;">MÉDIA DOL</span> <span style="color:#00f2ff; font-size: 16px;">{res['medio']:.2f}</span></div><div class="calc-row"><span style="color:#d4a017;">P. JUSTO</span> <span style="color:#ffffff; font-size: 16px; font-weight: bold;">{res['fraja']:.2f}</span></div><div class="calc-row" style="border-bottom: none;"><span style="color:#ff4d4d;">SPREED</span> <span style="color:#00f2ff; font-size: 16px; font-weight: bold;">{res['spreed']:.2f}</span></div></div>""", unsafe_allow_html=True)
