...
st.set_page_config(
    layout="wide",
    page_title="BAIR - TERMINAL DOLLAR",
...
def carregar_historico_dolfut_diario():
  data_hoje = datetime.now(pytz.timezone("America/Sao_Paulo")).strftime(
      "%Y-%m-%d"
  )
...
if "max_madr_mem" not in st.session_state:
  st.session_state.max_madr_mem = max_madr_salvo
if "min_madr_mem" not in st.session_state:
  st.session_state.min_madr_mem = min_madr_salvo

max_init, min_init, close_init = carregar_historico_dolfut_diario()
if "dolfut_max_auto" not in st.session_state:
  st.session_state.dolfut_max_auto = max_init
if "dolfut_min_auto" not in st.session_state:
  st.session_state.dolfut_min_auto = min_init
if "dolfut_close_auto" not in st.session_state:
  st.session_state.dolfut_close_auto = close_init

if "last_spot_max" not in st.session_state:
  st.session_state.last_spot_max = 0.0
if "last_spot_min" not in st.session_state:
  st.session_state.last_spot_min = float("inf")

if "c_spot_fech_val" not in st.session_state:
  st.session_state.c_spot_fech_val = 0.0
if "c_du_val" not in st.session_state:
  st.session_state.c_du_val = 22
if "t_br_val" not in st.session_state:
  st.session_state.t_br_val = 14.00
if "t_us_val" not in st.session_state:
...
def fetch(s):
  ...
  try:
    ...
    try:
      ...
      cl = float(fi.get("previousClose", 0) or fi.get("previous_close", 0))
      op = float(fi.get("open", 0))
      mx = float(fi.get("dayHigh", 0) or fi.get("day_high", 0))
      mn = float(fi.get("dayLow", 0) or fi.get("day_low", 0))
    except:
      pass
...
def calcular_k97_total(spreed_do_dia, spot_data, ewz_data):
  try:
    if not spot_data or not ewz_data:
      return None
    ...
    v_dxy = (
        ((dxy_data["at"] / dxy_data["cl"]) - 1) if dxy_data["cl"] > 0 else 0
    )
    ...
    salvar_historico_dolfut_diario(
    ...
    df_close = (
        st.session_state.dolfut_close_auto
        if st.session_state.dolfut_close_auto > 0
        else (spot_data["cl"] * spreed_do_dia)
    )
    df_var = (
        ((df_price / df_close) - 1) * 100
        if df_close > 0
        else (v_spot_pct * 100)
    )
...
with st.sidebar:
  ...
  with st.expander("CALCULAR FATOR", expanded=False):
    c_spot_fech = st.number_input(
        "FECH SPOT:", value=st.session_state.c_spot_fech_val, format="%.3f"
    )
    c_du = st.number_input(
        "DIAS ÚTEIS (DU):", value=st.session_state.c_du_val, step=1
    )
    t_br = st.number_input(
        "JUROS BRL (%):", value=st.session_state.t_br_val, format="%.2f"
    )
    t_us = st.number_input(
        "JUROS USD (%):", value=st.session_state.t_us_val, format="%.2f"
    )
  ...
  i_div = st.number_input(
      "FRP (FATOR MULTIPLICADOR):",
      value=st.session_state.div_spreed_mem,
      format="%.4f",
  )

  i_max_madr = st.number_input(
      "MAX MADRUGADA:", value=st.session_state.max_madr_mem, format="%.2f"
  )
  i_min_madr = st.number_input(
      "MIN MADRUGADA:", value=st.session_state.min_madr_mem, format="%.2f"
  )
...
# =============================================================================
while True:
  tz_sp, tz_ny, tz_ld, tz_utc = (
      pytz.timezone("America/Sao_Paulo"),
      pytz.timezone("America/New_York"),
      pytz.timezone("Europe/London"),
      pytz.utc,
  )
  spot_live, ewz_live, us10y_live = (
      fetch("USDBRL=X"),
      fetch("EWZ"),
      fetch("^TNX"),
  )
  ...
  with placeholder.container():
    ...
    if res:
      ...
      with c1:
        st.markdown(
            '<div class="section-title">MONITORAMENTO DA GRADE PRINCIPAL</div>',
            unsafe_allow_html=True,
        )
        ...
        for lbl, sym in outros.items():
          ...
          if d:
            ...
            l_a = st.session_state.last_p.get(lbl, p_v)
            cl_a = "f-up" if p_v > l_a else "f-dn" if p_v < l_a else ""
            st.session_state.last_p[lbl] = p_v
            var = (
                ((d["at"] / d["cl"]) - 1) * 100 if d["cl"] > 0 else 0
            )

            cl_max = (
                "f-up"
                if lbl == "DOLSPOT"
                and st.session_state.last_spot_max > 0
                and d["mx"] > st.session_state.last_spot_max
                else ""
            )
            cl_min = (
                "f-dn"
                if lbl == "DOLSPOT"
                and st.session_state.last_spot_min < float("inf")
                and d["mn"] < st.session_state.last_spot_min
                else ""
            )
            if lbl == "DOLSPOT":
              st.session_state.last_spot_max, st.session_state.last_spot_min = (
                  d["mx"],
                  d["mn"],
              )
        ...
        conteudo_verde = (
            f'<span class="txt-interno-tom-vermelho">{var_da_barra_txt}</span>'
            if res["p_v"] > 0
            else "&nbsp;"
        )
        conteudo_vermelho = (
            f'<span class="txt-interno-tom-verde">{var_da_barra_txt}</span>'
            if res["p_r"] > 0
            else "&nbsp;"
        )
        ...
        render_barra = (
            ...
            '        <span style="color:#00ff88; width:15%;'
            ' text-align:left;">-1.00%</span>'
            '        <span style="color:#00ff88; width:15%;'
            ' text-align:left;">-0.66%</span>'
            '        <span style="color:#00ff88; width:15%;'
            ' text-align:left;">-0.33%</span>'
            '        <span style="color:#ffffff; width:10%;'
            ' text-align:center;">0</span>'
            '        <span style="color:#ff4d4d; width:15%;'
            ' text-align:right;">+0.33%</span>'
            '        <span style="color:#ff4d4d; width:15%;'
            ' text-align:right;">+0.66%</span>'
            '        <span style="color:#ff4d4d; width:15%;'
            ' text-align:right;">+1.00%</span>'
            "    </div>"
            ...
            "        </div>"
            '        <div class="bar-side">'
            '            <div class="fill-red" style="width: '
            + p_r_val
            + '%;">'
            + conteudo_vermelho
            + "</div>"
            ...
            + "</span>"
            "    </div>"
            '    <div class="sinal-indicator" style="color:'
            + res["seta_cor"]
            + '; min-height:18px;">'
            "        "
            + sinal_txt
        ...
        if media_term <= -0.50:
          c_bf = "active-bf"
        elif media_term < -0.20:
          c_b = "active-b"
        elif media_term <= 0.20:
          c_n = "active-n"
        elif media_term < 0.50:
          c_a = "active-a"
        else:
          c_af = "active-af"
      ...
      with c2:
        ...
        st.markdown(
            f'''<div class="calc-panel">
                    ...
                    <div class="calc-row" style="border-bottom: none;"><span style="color:#00BFFF;">SPREAD T</span> <span style="color:#ffffff;">{res['spreed_t']:.2f}</span></div></div>''',
            unsafe_allow_html=True,
        )

        st.markdown(
            f'''<div class="calc-panel" style="text-align:center; border: 1.5px solid {res['cor_ind']}; padding-bottom:6px;">
      ...
      st.markdown(
          f'<div class="ticker-wrapper"><div class="ticker-text">{" • ".join(ticker_items)}</div></div>',
          unsafe_allow_html=True,
      )
