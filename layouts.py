...
def fetch(s):
    try:
        t = yf.Ticker(s)
        if s == "^TNX": d = yf.download(s, period="1d", interval="1m", prepost=True, progress=False)
        else: d = t.history(period="1d", interval="1m", prepost=True)
        if d.empty: return st.session_state.market_data.get(s)
        ref_close = t.info.get('previousClose')
        if s == "^TNX": ref_close = d['Open'].iloc[0]
...
def calcular_k97_total(div_spreed, p_ewz_atual, eixo_dol, spot_data, us10y_data):
    try:
        ...
        # --- CÁLCULOS DE FORÇA ---
        val_x, val_y = eixo_dol - (eixo_dol - media_pura_barra - folga), eixo_dol + (eixo_dol - media_pura_barra + folga)
        alvo_low, alvo_high = spot_data['mn'] + (eixo_dol - val_x), spot_data['mx'] + (val_y - eixo_dol)
        dist_base_barra, diff = abs(eixo_dol - media_pura_barra) + folga, spot_data['at'] - eixo_dol
        p_v, p_r = 0, 0
        ...
        var_us10y = (us10y_data['at'] / us10y_data['cl']) - 1 if (us10y_data and us10y_data['cl'] > 0) else 0
        ...
        v_ewz = ((p_ewz_atual / st.session_state.market_data.get("EWZ", {}).get('cl', 1)) - 1) if st.session_state.market_data.get("EWZ", {}).get('cl', 1) > 0 else 0
        ...
        return {
            ...
            "piscando": piscando, "max_grade": max_original, "min_grade": min_original, "alvo_low": alvo_low, 
            "alvo_high": alvo_high, "spreed_t": amp
        }
    except: return None
...
while True:
    ...
    with placeholder.container():
        ...
        if res:
            # VARIAÇÃO DO DOLB3 CALCULADA PELO NOVO AXIS FUT
            ...
            with c1:
                ...
                html += f"<tr><td class='asset-name'>DOLFUT</td><td class='price-col'>{(d_c/1000):.4f}</td><td>{(a_dol/1000):.4f}</td><td>{(a_dol/1000):.4f}</td><td>{(res['max_grade']/1000):.4f}</td><td>{(res['min_grade']/1000):.4f}</td><td style='color:{("#00ff00" if v_f >= 0 else "#ff4d4d")}; font-weight:bold;'>{v_f:+.2f}%</td></tr>"
            ...
            with c2:
                ...
                st.markdown(f'''<div class="calc-panel"><div class="calc-row txt-red"><span>MAX FUT 5</span> <span>{res['max_fut_5']:.2f}</span></div><div class="calc-row txt-red"><span>MAX FUT 3</span> <span>{res['max_fut_3']:.2f}</span></div><div style="text-align:center; padding: 4px; color: #00f2ff; font-size: 10px; font-weight: bold; border-top:1px solid #444; border-bottom:1px solid #444;">AXIS: {a_dol:.2f}</div><div class="calc-row txt-green"><span>MIN FUT 3</span> <span>{res['min_fut_3']:.2f}</span></div><div class="calc-row txt-green" style="border-bottom: none;"><span>MIN FUT 5</span> <span>{res['min_fut_5']:.2f}</span></div></div>''', unsafe_allow_html=True)
                st.markdown(f'''<div class="calc-panel"><div class="calc-row" style="border-bottom:none; padding-bottom:0px;"><span style="color:#ffffff;">DOLB3</span> <span style="color:#00f2ff;">{res['vivo']:.2f}</span></div><div style="text-align:right; font-size:9px; padding-right:6px; color:{("#00ff00" if var_dolb3_axis >= 0 else "#ff4d4d")}; font-weight:bold; margin-bottom:4px;">{var_dolb3_axis:+.2f}%</div><div class="calc-row"><span style="color:#d4a017;">PREÇO JUSTO</span> <span style="color:#ffffff;">{res['fraja']:.2f}</span></div><div class="calc-row" style="border-bottom: none;"><span style="color:#00BFFF;">SPREED T</span> <span style="color:#ffffff;">{res['spreed_t']:.2f}</span></div></div>''', unsafe_allow_html=True)
    time.sleep(5)
