...
# Memória de estado para a nova histerese tripla por graus
...
def calcular_k97_total(spreed_do_dia, spot_data, ewz_data):
    try:
        ...
        # Gatilhos do Indicador de Reversão MANTIDOS INTACTOS
        ...
        # Montagem Visual dos Alertas Dinâmicos
        ...
        # Atribuição do valor numérico do painel inferior do Indicador de Reversão
        if spot_data['at'] >= dolar_medio:
            ind_val = spot_data['at'] - gatilho_v
        else:
            ind_val = spot_data['at'] - gatilho_c
        cor_ind = "#ff4d4d" if st.session_state.grau_venda_ativo > 0 else "#00ff88" if st.session_state.grau_compra_ativo > 0 else "#00f2ff"
            
        # Preenchimento proporcional da barra (Escala de visualização normalizada em até 0.90% máximo)
        p_v, p_r = 0, 0
        if diff_media < 0:
    ...
    with placeholder.container():
        ...
        if res:
            ...
            with c1:
                ...
                # --- NOVA BARRA DE FORÇA MODIFICADA (RÉGUA TRIPLA DINÂMICA) ---
                st.markdown(f'''
            ...
            with c2:
                ...
                # Indicador de Reversão MANTIDO INTACTO (Cálculo Inalterado)
