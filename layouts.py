...
def fetch(s):
    try:
        t = yf.Ticker(s)
        # Forçando dados intraday (1m) para tempo real conforme solicitado
...
dolfut_price = fetch("BRL=F") # Referência para o preço do DOLFUT no terminal

# --- CÁLCULOS SOLICITADOS ---
spreed = (spot_live['mx'] - spot_live['mn']) / 8
max_fut = a_dol + spot_live['mx'] + spreed
min_fut = a_dol - spot_live['mn'] + spreed
media_dol = (spot_live['mx'] + spot_live['mn']) / 2
...
with c_side:
    # BLOCO SETA VERMELHA (AJUSTADO)
    st.markdown(f"""
    <div class="calc-panel">
        <div class="calc-row" style="color:#ff4d4d;"><span>MAX FUT</span> <span>{max_fut:.2f}</span></div>
        <div style="text-align:center; padding: 25px; color: #00f2ff; font-size: 22px; font-weight: bold; margin: 5px 0;">AXIS: {a_dol:.2f}</div>
        <div class="calc-row" style="color:#00ff88; border-bottom: none;"><span>MIN FUT</span> <span>{min_fut:.2f}</span></div>
    </div>
    """, unsafe_allow_html=True)

    # BLOCO SETA VERDE (AJUSTADO)
    st.markdown(f"""
    <div class="calc-panel">
        <div class="calc-row" style="padding: 10px 8px;"><span style="color:#ffffff;">DOLFUT</span> <span style="color:#00f2ff; font-size: 16px; font-weight: 950;">{dolfut_price['at']:.2f}</span></div>
        <div class="calc-row" style="border-bottom: none;"><span style="color:#ffff00;">MÉDIA DOL</span> <span style="color:#00f2ff; font-size: 16px;">{media_dol:.2f}</span></div>
    </div>
    """, unsafe_allow_html=True)
