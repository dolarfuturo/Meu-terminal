...
def calcular_k97_total(p_ewz_at, p_ewz_cl, p_spot_at, p_spot_cl, eixo_dol, a_ewz):
    try:
        if p_ewz_cl == 0 or p_spot_cl == 0: return None
        
        # 1. VARIAÇÕES REAIS DO DIA (SEM EIXO)
        var_spot = ((p_spot_at / p_spot_cl) - 1) * 100
        var_ewz_inv = ((p_ewz_at / p_ewz_cl) - 1) * -100 # Invertido para o dólar
        
        # 2. CÁLCULO PONDERADO DA VARIAÇÃO DO FUTURO
        var_dolfut = (var_spot * 0.6) + (var_ewz_inv * 0.4)
        
        # 3. PREÇO VIVO (Única baseada no Eixo)
        dolar_vivo = eixo_dol * (1 + (var_dolfut / 100))
        
        # Alvos fixos de exaustão (baseados no eixo do EWZ para manter a barreira)
        v_alvo = 0.65 # Fator de ajuste para os muros
        alvo_max = eixo_dol * (1 + v_alvo/100)
        alvo_min = eixo_dol * (1 - v_alvo/100)
        
        return {
            "vivo": dolar_vivo, "var_f": var_dolfut, "max": alvo_max, "min": alvo_min,
...
with st.sidebar:
    ...
    with st.form("ajuste_vars"):
        a_ewz = st.number_input("AXIS EWZ (REF):", value=32.50, format="%.2f")
...
res = calcular_k97_total(ewz_live['at'], ewz_live['cl'], spot_live['at'], spot_live['cl'], a_dol, a_ewz)
...
if res:
    ...
    with c_main:
        ...
        # DOLFUT (Calculado sobre o Eixo)
        html_table += f"<tr><td class='asset-name'>DOLFUT</td><td class='price-col'>{(res['vivo']/1000):.4f}</td><td>{(a_dol/1000):.4f}</td><td>{(a_dol/1000):.4f}</td><td>{(res['max']/1000):.4f}</td><td>{(res['min']/1000):.4f}</td><td style='color:{("#00ff00" if res['var_f'] >= 0 else "#ff4d4d")}; font-weight:bold;'>{res['var_f']:+.2f}%</td></tr>"
        ticker = [f"DOLFUT: {res['var_f']:+.2f}%"]
        
        outros = {"DOLSPOT": "USDBRL=X", "DXY": "DX-Y.NYB", "EWZ": "EWZ", "EUR/USD": "EURUSD=X", "XAU/USD": "GC=F", "BRENT": "BZ=F"}
        for lbl, sym in outros.items():
            ...
            color = "#00ff00" if v >= 0 else "#ff4d4d"
            html_table += f"<tr><td class='asset-name'>{lbl}</td><td class='price-col'>{d['at']:{f}}</td><td>{d['cl']:{f}}</td><td>{d['op']:{f}}</td><td>{d['mx']:{f}}</td><td>{d['mn']:{f}}</td><td style='color:{color}; font-weight:bold;'>{v:+.2f}%</td></tr>"
            ticker.append(f"{lbl}: {v:+.2f}%")
            
        st.markdown(html_table + "</tbody></table></div>", unsafe_allow_html=True)

    with c_side:
        # PAINEL DE ALVOS (AXIS DOLFUT)
        ...
        st.markdown(f"""<div class="calc-panel"><div class="calc-row" style="padding: 10px 8px;"><span style="color:#ffffff;">DOLFUT</span> <span style="color:#00f2ff; font-size: 16px; font-weight: 950;">{res['vivo']:.2f}</span></div><div class="calc-row" style="border-bottom: none;"><span style="color:#d4a017;">VAR PONDERADA</span> <span style="color:#ffffff; font-size: 16px; font-weight: bold;">{res['var_f']:+.2f}%</span></div></div>""", unsafe_allow_html=True)
