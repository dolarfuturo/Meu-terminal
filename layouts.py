...
def calcular_logica_bair(p_ewz, p_spot, eixo_dol):
    try:
        if p_ewz['cl'] == 0 or p_spot['cl'] == 0: return None
        
        # 1. VARIAÇÕES REAIS DO DIA (Yahoo Close)
        v_spot = ((p_spot['at'] / p_spot['cl']) - 1) * 100
        v_ewz_inv = ((p_ewz['at'] / p_ewz['cl']) - 1) * -100 # Inverte EWZ
        
        # 2. CÁLCULO DA VARIAÇÃO PONDERADA (60% SPOT / 40% EWZ)
        var_final_dolfut = (v_spot * 0.6) + (v_ewz_inv * 0.4)
        
        # 3. DOLFUT VIVO BASEADO NO EIXO AXIS
        dolar_vivo = eixo_dol * (1 + (var_final_dolfut / 100))
        
        # Alvos de exaustão (Muros em 0.65% do eixo)
        muro = 0.65
        alvo_max = eixo_dol * (1 + muro/100)
        alvo_min = eixo_dol * (1 - muro/100)
        
        return {
            "vivo": dolar_vivo, "var": var_final_dolfut, "max": alvo_max, "min": alvo_min,
...
res = calcular_logica_bair(ewz_live, spot_live, a_dol)
...
if res:
    ...
    with c_side:
        ...
        st.markdown(f"""<div class="calc-panel"><div class="calc-row" style="padding: 10px 8px;"><span style="color:#ffffff;">DOLFUT</span> <span style="color:#00f2ff; font-size: 16px; font-weight: 950;">{res['vivo']:.2f}</span></div><div class="calc-row" style="border-bottom: none;"><span style="color:#d4a017;">VAR PONDERADA</span> <span style="color:#ffffff; font-size: 16px; font-weight: bold;">{res['var']:+.2f}%</span></div></div>""", unsafe_allow_html=True)
