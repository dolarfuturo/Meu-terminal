...
def calcular_k97_total(eixo_ewz, p_ewz, p_spot, eixo_dol):
    try:
        # 1. CÁLCULO DAS VARIAÇÕES REAIS
        v_spot = ((p_spot['at'] / p_spot['cl']) - 1) * 100
        v_ewz_inv = ((p_ewz['at'] / eixo_ewz) - 1) * -100 # Inverte o EWZ
        
        # 2. VARIAÇÃO DO DOLFUT (Sua fórmula: Spot*0.6 + EWZ_Inv*0.4)
        var_dolfut = (v_spot * 0.6) + (v_ewz_inv * 0.4)
        dolar_vivo = eixo_dol * (1 + (var_dolfut / 100))
        
        # 3. P. JUSTO (Apenas EWZ Invertido)
        dolar_fraja = eixo_dol * (1 + (v_ewz_inv / 100))
        
        # 4. MÉDIA DO DIA (Média EWZ Invertida)
        ewz_medio_dia = (p_ewz['mx'] + p_ewz['mn']) / 2
        v_med_inv = ((ewz_medio_dia / eixo_ewz) - 1) * -100
        dolar_medio = eixo_dol * (1 + (v_med_inv / 100))
        
        # 5. MUROS (Exaustão 1.5 conforme original, mas invertendo os limites do EWZ)
        ...
        return {
            ...
            "max": alvo_max, "min": alvo_min, "var_calc": var_dolfut,
...
if res:
    ...
    with c_main:
        ...
        # LINHA DOLFUT COM A VARIAÇÃO CALCULADA PELA SUA FÓRMULA
        html_table += f"<tr><td class='asset-name'>DOLFUT</td><td class='price-col'>{(res['vivo']/1000):.4f}</td><td>{(a_dol/1000):.4f}</td><td>{(a_dol/1000):.4f}</td><td>{(res['max']/1000):.4f}</td><td>{(res['min']/1000):.4f}</td><td style='color:{("#00ff00" if res['var_calc'] >= 0 else "#ff4d4d")}; font-weight:bold;'>{res['var_calc']:+.2f}%</td></tr>"
        ticker = [f"<span style='color:#fff;'>DOLFUT:</span> <span style='color:{("#00ff00" if res['var_calc'] >= 0 else "#ff4d4d")};'>{res['var_calc']:+.2f}%</span>"]
        ...
        for lbl, sym in outros.items():
            # Evita fetch repetido se já tiver os dados
    ...
    with c_side:
        ...
        # RESUMO FINAL COM O PREÇO VIVO (CÁLCULO NOVO)
