...
def calcular_k97_total(eixo_ewz, p_ewz_atual, max_ref_ewz, min_ref_ewz, eixo_dol):
    # Variações baseadas no preço atual vs Eixo
    ...
    # Projeções de Alvo (Onde o dólar vai se o EWZ bater na Max/Min de referência)
...
with st.sidebar:
    ...
    mx_ref = st.number_input("MAX EIXO (REF):", value=float(mx_sug), format="%.2f")
    mn_ref = st.number_input("MIN EIXO (REF):", value=float(mn_sug), format="%.2f")
    st.divider()
...
if ewz_live:
    ...
    # Linhas Fixas K97 - Usando as referências do ajuste
