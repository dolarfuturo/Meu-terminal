...
# =============================================================================
# BLOCO 3: FUNÇÕES DE DADOS E CÁLCULO
# =============================================================================
def fetch(s):
...
with st.sidebar:
    ...
    if st.button("SALVAR CONFIGURAÇÕES"):
        st.session_state.div_spreed_mem, st.session_state.max_madr_mem, st.session_state.min_madr_mem = i_div, i_max_madr, i_min_madr
...
# =============================================================================
# BLOCO 5: INTERFACE (SEM WHILE TRUE)
# =============================================================================
...
if res:
    ...
    with c1:
        ...
        render_barra = f'''<div class="bar-wrapper-full"><div class="force-scale-top"><span style="color:#00ff88; width:15%; text-align:left;">-1.05%</span><span style="color:#00ff88; width:15%; text-align:left;">-0.70%</span><span style="color:#00ff88; width:15%; text-align:left;">-0.35%</span><span style="color:#ffffff; width:10%; text-align:center;">0</span><span style="color:#ff4d4d; width:15%; text-align:right;">+0.35%</span><span style="color:#ff4d4d; width:15%; text-align:right;">+0.70%</span><span style="color:#ff4d4d; width:15%; text-align:right;">+1.05%</span></div><div class="force-container-dual"><div class="center-line"></div><div class="bar-side"><div class="fill-green" style="width: {p_v_val}%;">{('<span class="txt-interno-tom-vermelho">' + "{:+.2f}%".format(res['pct_afastamento']) + '</span>') if res['p_v'] > 0 else "&nbsp;"}</div></div><div class="bar-side"><div class="fill-red" style="width: {p_r_val}%;">{('<span class="txt-interno-tom-verde">' + "{:+.2f}%".format(res['pct_afastamento']) + '</span>') if res['p_r'] > 0 else "&nbsp;"}</div></div></div><div class="force-scale-bottom"><span style="width:15%; text-align:left;">{"{:.4f}".format(res['p_c3_v'] / 1000)}</span><span style="width:15%; text-align:left;">{"{:.4f}".format(res['p_c2_v'] / 1000)}</span><span style="width:15%; text-align:left;">{"{:.4f}".format(res['p_c1_v'] / 1000)}</span><span style="color:#ffffff; width:10%; text-align:center;">&nbsp;</span><span style="width:15%; text-align:right;">{"{:.4f}".format(res['p_v1_v'] / 1000)}</span><span style="width:15%; text-align:right;">{"{:.4f}".format(res['p_v2_v'] / 1000)}</span><span style="width:15%; text-align:right;">{"{:.4f}".format(res['p_v3_v'] / 1000)}</span></div><div class="sinal-indicator" style="color:{res["seta_cor"]}; min-height:18px;">{res["seta"] if res["seta"] else "&nbsp;"}</div></div>'''
    ...
    with c2:
        ...
        # Correção aqui: Quebra de string para evitar o erro do interpretador
        st.markdown(f'''
