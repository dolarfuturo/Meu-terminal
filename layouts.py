...
def calcular_k97_total(eixo_ewz, p_ewz_atual, max_ewz, min_ewz, eixo_dol, spot_data):
    try:
        ...
        # --- BLOCO SETA VERDE: MEDIA DOL ---
        dolar_medio = ((spot_data['mx'] + spot_data['mn']) / 2) * 1000
        
        # --- BLOCO SETA VERMELHA: MAX/MIN FUT (AXIS + VAR SPOT + SPREAD) ---
        # Spread = (Max - Min) / 8 do Spot
        spread_spot = (spot_data['mx'] - spot_data['mn']) / 8
        
        # Alvos baseados na variação em pontos do spot somada ao AXIS
        # (SpotMax - SpotClose) transformado em pontos de dólar
        pontos_max = (spot_data['mx'] - spot_data['cl']) * 1000
        pontos_min = (spot_data['cl'] - spot_data['mn']) * 1000
        val_spread = spread_spot * 1000

        alvo_max = eixo_dol + pontos_max + val_spread
        alvo_min = eixo_dol - pontos_min + val_spread
        
        return {
            "vivo": eixo_dol * (1 + v_final),
            "fraja": eixo_dol * (1 + (v_final / 4)),
            "medio": dolar_medio, 
...
if res:
    ...
    with c_main:
        ...
        for lbl, sym in {"DOLSPOT": "USDBRL=X", "DXY": "DX-Y.NYB", "EWZ": "EWZ", "GBP/USD": "GBPUSD=X", "JPY/USD": "JPYUSD=X", "EUR/USD": "EURUSD=X", "XAU/USD": "GC=F", "PETROLEO BRENT": "BZ=F"}.items():
